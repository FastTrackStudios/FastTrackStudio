...
fn SpectrumView(props: SpectrumViewProps) -> Element {
    ...
    let ulay_painter = use_hook(|| {
        make_canvas(SpectrumPainter::new(
            props.r_state.clone(),
            ulay_config.clone(),
        ))
    });
    ...
    let slope_items: Vec<String> = SLOPES
        .iter()
        .map(|(_, s)| format!("{} dB/oct", s))
        .collect();
...
fn SpectrographView(props: SpectrographViewProps) -> Element {
    let painter = use_hook(|| {
        make_canvas(SpectrographPainter::new(
            props.state.clone(),
            SpectrographConfig::default(),
        ))
    });
    rsx! {
...
fn KMeterView(props: KMeterViewProps) -> Element {
    let painter_l = use_hook(|| {
        make_canvas(KMeterPainter::new(
            props.l_state.clone(),
            KMeterConfig::default(),
        ))
    });
    let painter_r = use_hook(|| {
        make_canvas(KMeterPainter::new(
            props.r_state.clone(),
            KMeterConfig::default(),
        ))
...
fn GoniometerView(props: GoniometerViewProps) -> Element {
    let painter = use_hook(|| {
        make_canvas(GoniometerPainter::new(
            props.state.clone(),
...
fn BitsView(props: BitsViewProps) -> Element {
    let painter = use_hook(|| {
        make_canvas(BitMeterPainter::new(
            props.state.clone(),
            BitMeterConfig::default(),
        ))
    });
    rsx! {
        VelloCanvas { painter: painter, fill: true, background: true }

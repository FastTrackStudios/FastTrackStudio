...
The runtime DI engine (see [idioms §6–7](@/architect/architecture/idioms.md)):
...
| `Schedule` / `retry` / `repeat` | `architect::schedule` (feature `schedule`) | composable retry/repeat policies — backoff, jitter, caps; see [scheduling](@/architect/architecture/scheduling.md) |
...
See [The architect pattern](@/architect/architecture/pattern.md) for the full
...
see [streams](@/architect/architecture/streams.md).
...
[feature flags](@/architect/architecture/features.md)). Runtime-specific

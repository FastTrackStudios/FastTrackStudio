...
| `rt` | — | rtrb | `architect::rt` — wait-free SPSC ring (`rt_channel`) bridging real-time threads (audio callbacks) to `PubSub`; drop-on-full, drained at UI rate. See [server push](@/architect/architecture/streams.md). |

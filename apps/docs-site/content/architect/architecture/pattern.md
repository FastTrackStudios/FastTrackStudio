...
- `ExampleRepoLayer` — a `Layer` service token (+ a `<snake>_repo_layer`
  ...
  See [idioms §6](@/architect/architecture/idioms.md).
...
[idioms §7 — inject the transport](@/architect/architecture/idioms.md) and the
[end-to-end walkthrough](@/architect/getting-started/walkthrough.md).

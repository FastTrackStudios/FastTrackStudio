...
[in-process link](@/architect/architecture/local.md)) as the CRUD calls.
...
[Entity derive](@/architect/architecture/pattern.md) emits the whole story:
...
the optimistic [`Store`](@/architect/architecture/optimistic.md): `apply` is a

...
[`pattern.md`](@/architect/architecture/pattern.md) covers the server side — one
...
[optimistic state](@/architect/architecture/optimistic.md).
...
- **Transport** (`transport.rs`) — `Transport::Remote(url)` vs
  ...
  (wrapped in [`schedule::retry`](@/architect/architecture/scheduling.md)).
...
1. **Navigation is Dioxus.** `Link { to: Route::… }` / `nav.push` — never an
   ...
   [idioms §4](@/architect/architecture/idioms.md)).
...
4. **Writes are optimistic.** Mutations patch the store instantly and
   reconcile/roll back ([optimistic state](@/architect/architecture/optimistic.md)).

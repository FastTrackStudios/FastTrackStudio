...
pub(crate) mod embedded {
    //! Everything this site embeds at build time, under one path.
    //!
    //! The guides vault (VAULT, MEDIA_ASSETS) is generated by this crate's
    //! own build.rs from docs/guides. The keybind configuration comes from
    //! the crate that OWNS it -- `daw`'s reaper-input-config -- because a
    //! build script cannot reach across a repo boundary to read it.
    include!(concat!(env!("OUT_DIR"), "/input_profiles.rs"));
    pub use reaper_input_config::{
        EmbeddedNamed, EmbeddedProfile, EmbeddedSection, OVERLAYS, PROFILES, WORKFLOWS,
    };
}

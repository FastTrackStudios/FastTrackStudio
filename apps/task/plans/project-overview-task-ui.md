...
- fts-ui primitives only; theme tokens (`bg-card`, `text-muted-foreground`, …), never hex; dark mode default.
...
2. **Kanban drag-drop**: column drop → `SetStatus`. Use the fts-ui drag primitive if present (check catalog: `dx components` — do NOT hand-roll listeners); else reuse the gantt port's pointer-drag pattern.
...
## New components (all dumb, fts-ui only)
| Component | Crate | Props (sketch) |
|---|---|---|
| `StatCard` | `crates/ui` or fts-ui | `label`, `value`, `progress: Option<f32>` |
| `ProgressBar` | fts-ui (reusable) | `percent: f32`, `variant` |
...
`project-ui` / `milestone-ui` are new feature-trio UI crates mirroring `task-ui` (dumb, fts-ui). `crates/ui` pages own the data + compose them.

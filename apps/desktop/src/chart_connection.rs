...
async fn connect_with_retry() {
    ...
    loop {
        retry_count += 1;
        info!(
            "[Chart Connection] Attempting to connect (attempt {})...",
            retry_count
        );
        ...
        match try_connect().await {
            Ok(disconnect_rx) => {
                info!(
                    "[Chart Connection] ✅ Successfully connected on attempt {}",
                    retry_count
                );
                retry_count = 0;
...
async fn try_connect() -> Result<tokio::sync::oneshot::Receiver<()>> {
    let endpoint =
        get_shared_endpoint().ok_or_else(|| anyhow::anyhow!("Shared endpoint not initialized"))?;

    let endpoint_addr = get_reaper_endpoint_addr()?
        .ok_or_else(|| anyhow::anyhow!("REAPER endpoint address not found"))?;

    info!(
        "[Chart Connection] Connecting to REAPER at {:?}",
        endpoint_addr
    );
                    ...
                    match msg {
                        ChartUpdateMessage::ChartChanged(changed) => {
                            ...
                            if let Some(chart_state) = CHART_STATE.get() {
                                chart_state
                                    .write()
                                    .insert(changed.project_name.clone(), changed.chart.clone());
                            }

                            // Update global Dioxus signal for UI components
                            #[cfg(feature = "dioxus")]
                            {
                                use fts::chords::CHART_STATE_SIGNAL;
                                CHART_STATE_SIGNAL
                                    .write()
                                    .insert(changed.project_name, changed.chart);
                            }
                        }
                    }
                }

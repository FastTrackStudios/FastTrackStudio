...
pub async fn set_track_mute(
    ...
    info!(
        "[Track Commands] set_track_mute called: project={}, track={}, muted={}",
        project_id, track_index, muted
    );
    ...
    tokio::spawn(async move {
        let result = if let Some(storage) = TRACKS_API.get() {
            ...
            if let Some(api) = guard.as_ref() {
                match api
                    .set_track_mute(project_id_clone, track_index, muted)
                    .await
                {
                    Ok(Ok(())) => {
                    ...
                    Ok(Err(e)) => {
                        warn!(
                            "[Track Commands] ❌ Failed to execute set_track_mute: {}",
                            e
                        );
                        Err(e)
                    }
                    Err(e) => {
                        warn!(
                            "[Track Commands] ❌ RPC error sending set_track_mute: {}",
                            e
                        );
...
pub async fn set_track_solo(
    ...
    info!(
        "[Track Commands] set_track_solo called: project={}, track={}, solo_mode={:?}",
        project_id, track_index, solo_mode
    );
    let (tx, rx) = oneshot::channel();
    let project_id_clone = project_id.clone();
    ...
    tokio::spawn(async move {
        let result = if let Some(storage) = TRACKS_API.get() {
            ...
            if let Some(api) = guard.as_ref() {
                match api
                    .set_track_solo(project_id_clone, track_index, solo_mode)
                    .await
                {
                    Ok(Ok(())) => {
                    ...
                    Ok(Err(e)) => {
                        warn!(
                            "[Track Commands] ❌ Failed to execute set_track_solo: {}",
                            e
                        );
                        Err(e)
                    }
                    Err(e) => {
                        warn!(
                            "[Track Commands] ❌ RPC error sending set_track_solo: {}",
                            e
                        );
                        Err(format!("RPC error: {}", e))
                    }

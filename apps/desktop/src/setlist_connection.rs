...
pub(crate) static SETLIST_API: OnceLock<tokio::sync::Mutex<Option<SetlistStreamApi>>> =
    OnceLock::new();
...
#[cfg(not(target_arch = "wasm32"))]
#[deprecated(
    note = "SetlistStreamApi no longer implements Clone. Use a different approach to access the API."
)]
...
async fn connect_with_retry() {
    ...
    loop {
        retry_count += 1;
        info!(
            "[Setlist Connection] Attempting to connect (attempt {})...",
            retry_count
        );
        ...
        match try_connect().await {
            Ok(disconnect_rx) => {
                info!(
                    "[Setlist Connection] ✅ Successfully connected on attempt {}",
                    retry_count
                );
            ...
            Err(e) => {
                if retry_count == 1 || retry_count.is_multiple_of(10) {
                    warn!(
                        "[Setlist Connection] Connection attempt {} failed: {}",
                        retry_count, e
                    );
                }
                ...
                *ACTIVE_SLIDE_INDEX.write() = None;
                ...
                info!(
                    "[Setlist Connection] Retrying in {} seconds...",
                    RETRY_INTERVAL_SECONDS
                );
...
async fn try_connect() -> Result<tokio::sync::oneshot::Receiver<()>> {
    ...
    let endpoint = get_shared_endpoint().ok_or_else(|| {
        anyhow::anyhow!("Shared endpoint not initialized. Call connect_to_reaper_setlist() first.")
    })?;
    ...
    let setlist_api = SetlistStreamApi::connect(endpoint.clone(), endpoint_addr).map_err(|e| {
        warn!(
            "[Setlist Connection] Failed to create connection wrapper: {}",
            e
        );
    ...
    let (mut structure_rx, mut active_indices_rx, mut tracks_rx, mut transport_rx) =
        ...
        .await
        {
            ...
            Ok(Err(e)) => {
                warn!(
                    "[Setlist Connection] ❌ Failed to connect/subscribe to streams: {}",
                    e
                );
                return Err(anyhow::anyhow!(
                    "Failed to connect/subscribe to streams: {}",
                    e
                ));
            }
            Err(_) => {
...
async fn handle_message(
    msg: fts::setlist::SetlistUpdateMessage,
    ...
    match msg {
        ...
        SetlistUpdateMessage::SongTracks { song_index, tracks } => {
            ...
            info!(
                "[Setlist Connection] ✅ Updated tracks for song {} ({} tracks)",
                song_index,
                tracks.len()
            );
        }
        SetlistUpdateMessage::SongTransport {
            ...
            debug!(
                "[Setlist Connection] Updated transport for song {}",
                song_index
            );
        }
            ...
            if let Some(mut setlist) = SETLIST_STRUCTURE.read().clone() {
                if let Some(existing_song) = setlist.songs.get_mut(song_index) {
                    ...
                    *SETLIST_STRUCTURE.write() = Some(setlist);
                    debug!(
                        "[Setlist Connection] Updated metadata for song {}",
                        song_index
                    );
                }
            }
    ...
    if last_log_guard.elapsed().as_secs() >= 10 {
        ...
        info!(
            "[Performance] IRPC receive: {} msg/s, UI updates: {} updates/s",

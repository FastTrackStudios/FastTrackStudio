...
fn Performance() -> Element {
    ...
    let on_seek_to_musical_position = {
        ...
        Callback::new(move |(song_idx, musical_pos): (usize, daw::primitives::MusicalPosition)| {
            let musical_pos_for_log = musical_pos.clone();
            spawn(async move {
                if let Err(e) = seek_to_musical_position(song_idx, musical_pos).await {
                    warn!("Failed to seek to song {} at musical position {}: {}", song_idx, musical_pos_for_log, e);
                }
            });
        })
    };
    ...
    #[cfg(target_arch = "wasm32")]
    let on_seek_to_musical_position = Callback::new(move |(_song_idx, _musical_pos): (usize, daw::primitives::MusicalPosition)| {});
...
#[component]
fn Chart() -> Element {
    use pdf::{render_pdf_page_with_text, get_pdf_info, RotationAngle, PdfPageData};
    ...
    let (total_pages, pdf_info) = use_memo(move || {
        if let Some(path) = pdf_path() {
            match get_pdf_info(&path.to_string_lossy()) {
                Ok((pages, title)) => {
                    tracing::info!("Successfully loaded PDF: {} ({} pages)", path.display(), pages);
                    (pages, title)
                }
                Err(e) => {
                ...
                spawn(async move {
                    ...
                    for page_idx in 0..3.min(total_pages) {
                        ...
                        match render_pdf_page_with_text(&path_clone.to_string_lossy(), page_idx, rotation) {
                            ...
                            Err(e) => {
                                let error_msg = format!("Failed to render page {}: {}", page_idx, e);
                                tracing::error!("{}", error_msg);
                    ...
                    tracing::info!("Finished loading PDF pages. Total loaded: {}", page_cache.read().len());
                });
            }
        }
    });
    ...
    let rendered_pages = use_memo(move || {
        ...
        tracing::debug!("Rendered pages memo: total_pages={}, cache_size={}", total_pages, cache.len());
        for page_idx in 0..total_pages {
            if let Some(page_data) = cache.get(&page_idx) {
                // Contamination check: verify page index matches
                if page_data.page_index != page_idx {
                    tracing::warn!("Page data mismatch detected! Expected page {}, got page {}", page_idx, page_data.page_index);
                    continue;
                }
            ...
            if pdf_path().is_none() {
                ...
                div {
                    ...
                    div {
                        ...
                        if rendered_pages().is_empty() && page_cache.read().len() > 0 {
                            div {
...
fn create_sample_setlist() -> Setlist {
    ...
    song1.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(
    ...
    song2.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(0.0, 120.0, None, Some((4, 4)), None, None, None));
    song2.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(8.0, 130.0, None, None, None, None, None));
    song2.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(24.0, 130.0, None, Some((3, 4)), None, None, None));
    song2.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(40.0, 130.0, None, Some((5, 4)), None, None, None));
    song2.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(56.0, 130.0, None, Some((4, 4)), None, None, None));
    ...
    song3.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(0.0, 120.0, None, Some((4, 4)), None, None, None));
    song3.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(28.0, 120.0, None, Some((3, 4)), None, None, None));
    song3.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(44.0, 120.0, None, Some((7, 8)), None, None, None));
    song3.tempo_time_sig_changes.push(TempoTimePoint::new_full_from_seconds(68.0, 140.0, None, Some((4, 4)), None, None, None));

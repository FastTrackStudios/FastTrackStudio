...
use crate::input::state::{Command, ActionType, ActionKey};
...
pub fn execute_command(command: &Command) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, action_type) in command.action_sequence.iter().enumerate() {
        ...
        } else if let Some(named_id) = medium_reaper.named_command_lookup(action_key.identifier.as_str()) {
            named_id
        } else {
        ...
        debug!("Executed action: {} ({} times)", action_key.identifier, repetitions);
    }

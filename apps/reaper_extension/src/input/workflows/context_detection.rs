...
use crate::input::constants::{
    AI_EDGE_THRESHOLD_PX, EDGE_THRESHOLD_PX, ENV_HIT_POINT, ENV_HIT_POINT_DOWN, ENV_HIT_POINT_LEFT,
    FADE_HANDLE_HEIGHT_PX, LOWER_HALF_THRESHOLD, MIDI_BLACK_KEYS_W, MIDI_LANE_DIVIDER_H,
    MIDI_NOTE_EDGE_HIT, MIDI_RULER_H,
};
...
pub fn detect_context_at_point(mouse_x: i32, mouse_y: i32) -> MouseContextResult {
    ...
    let (mcp_hwnd, mcp_is_container) = reaper_windows::get_mcp_wnd(&medium);
...
fn detect_midi_editor_context(
    ...
    if let Some(piano_view) = reaper_windows::get_piano_view(midi_editor, medium) {
        if hwnd_under_mouse == piano_view {
    ...
    if let Some(notes_view) = reaper_windows::get_notes_view(midi_editor, medium) {
        if hwnd_under_mouse == notes_view {
            ...
            let last_cc_lane = unsafe {
                low.MIDIEditor_GetSetting_int(midi_editor, c"last_clicked_cc_lane".as_ptr())
            };
            ...
            if last_cc_lane != -2 {
                ...
                if pt_client.y > notes_height - cc_area_estimate {
                    ...
                    if !take.is_null() {
                        if let Some((cc_ctx, cc_details)) = detect_midi_cc_at_point(
                            take,
                            pt_client.x,
                            pt_client.y,
                            notes_width,
            ...
            let take = unsafe { low.MIDIEditor_GetTake(midi_editor) };
            if !take.is_null() {
                if let Some((note_ctx, note_details)) = detect_midi_note_at_point(
                    take,
                    pt_client.x,
                    pt_client.y,
                    notes_width,
    ...
    // Check for CC selector (lane header) - this is to the left of the notes view
    // The CC selector is typically a small control ID area
    // SWS detects this via control ID WndControlIDs::midi_ccLaneSelector = 0x3F1
    let cc_selector_id: i32 = 0x3F1;
    let cc_selector = unsafe { swell.GetDlgItem(midi_editor, cc_selector_id) };
...
fn detect_midi_note_at_point(
    ...
    let start_ppq = unsafe { low.MIDI_GetPPQPosFromProjTime(take, start_time) };
    let end_ppq = unsafe { low.MIDI_GetPPQPosFromProjTime(take, end_time) };
    ...
    let v_zoom = notes_area_height as f64 / visible_notes as f64;

    // Mouse position in time
    let mouse_time = start_time + (mouse_x as f64) / h_zoom;
    let mouse_ppq = unsafe { low.MIDI_GetPPQPosFromProjTime(take, mouse_time) };

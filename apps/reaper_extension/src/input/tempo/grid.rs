...
pub fn position_at_mouse_cursor(_check_ruler: bool) -> Option<f64> {
    ...
    if let Some(arrange_hwnd) = crate::input::utils::get_arrange_wnd(reaper.medium_reaper()) {
    ...
    let (window, _segment, _details) =
        crate::input::mouse_context::get_mouse_cursor_context(reaper.medium_reaper());

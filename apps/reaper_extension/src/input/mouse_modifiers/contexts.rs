...
pub mod categories {
    ...
    pub fn fixed_lane() -> Vec<&'static str> {
        ALL_CONTEXTS.iter()
            .filter(|ctx| ctx.starts_with("MM_CTX_FIXEDLANE") || ctx.starts_with("MM_CTX_LINKEDLANE"))
            .copied()
            .collect()
    }
    ...
    pub fn project() -> Vec<&'static str> {
        ALL_CONTEXTS.iter()
            .filter(|ctx| ctx.starts_with("MM_CTX_MARKER") || ctx.starts_with("MM_CTX_REGION") || ctx.starts_with("MM_CTX_TEMPOMARKER"))
            .copied()
            .collect()
    }

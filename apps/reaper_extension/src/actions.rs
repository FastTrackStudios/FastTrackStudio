...
/// Type alias for tempo/time signature markers to improve readability
type TempoTimeSigMarker = (f64, f64, Option<(i32, i32)>);
            ...
            let play_state_str = match transport_result {
                Ok(transport) => {
                    ...
                    format!(
                        ...
                        is_playing, transport.tempo.bpm, time_pos, musical_pos
                    )
                }
            ...
            if markers.is_empty() {
                ...
                for (idx, marker) in markers.iter().enumerate() {
                    ...
                    let marker_msg = format!(
                        "  {}{}: {:.2}s ({})\n",
                        id_str, marker.name, time_pos, musical_pos
                    );
...
fn log_region_colors_handler() {
    ...
    reaper
        .show_console_msg(format!("Total bookmarks: {}\n\n", bookmark_count.total_count).as_str());
    ...
    if region_count == 0 {
        ...
        reaper.show_console_msg(
            format!("Found {} region(s) with color information.\n", region_count).as_str(),
        );
    }
...
pub fn build_setlist_from_projects_handler() {
    ...
    match reaper.build_setlist_from_open_projects(None) {
        Ok(setlist) => {
            ...
            reaper.show_console_msg(
                format!("Built setlist with {} song(s):\n\n", song_count).as_str(),
            );
            ...
            for (idx, song) in setlist.songs.iter().enumerate() {
                ...
                if !song.sections.is_empty() {
                    ...
                    for section in &song.sections {
                        let start_musical = section
                            .start_position
                            .as_ref()
                            .map(|pos| pos.musical_position_string())
                            .unwrap_or_else(|| "N/A".to_string());
                        let end_musical = section
                            .end_position
                            .as_ref()
                            .map(|pos| pos.musical_position_string())
                            .unwrap_or_else(|| "N/A".to_string());
    ...
    match reaper.build_setlist_from_open_projects(None) {
        Ok(setlist) => {
            ...
            if song_count == 0 {
                ...
                reaper.show_console_msg(
                    format!("Found {} song(s) in setlist:\n\n", song_count).as_str(),
                );
            ...
            if let Some(start_marker) = &song.song_region_start_marker {
                if let Some(end_marker) = &song.song_region_end_marker {
                    ...
                    reaper.show_console_msg(
                        format!(
                            "\nSong Region: {:.2}s ({}) - {:.2}s ({})\n",
                            start_time, start_musical, end_time, end_musical
                        )
                        .as_str(),
                    );
                }
...
fn read_tempo_time_sig_markers(project: &Project) -> Vec<TempoTimeSigMarker> {
    let reaper = Reaper::get();
    let medium_reaper = reaper.medium_reaper();
...
fn get_project_measure_offset(project: &reaper_high::Project) -> i32 {
    ...
    if let Some(offs_result) = medium_reaper.project_config_var_get_offs("projmeasoffs") {
        ...
        if let Some(addr) =
            medium_reaper.project_config_var_addr(project.context(), offs_result.offset)
        {
    ...
    match transport_adapter.read_transport() {
        Ok(transport) => {
            ...
            reaper.show_console_msg(
                format!(
                    "Musical Position: {}\n",
                    playhead_pos.musical_position_string()
                )
                .as_str(),
            );
            reaper
                .show_console_msg(format!("Project Measure Offset: {}\n", measure_offset).as_str());
            reaper.show_console_msg(
                format!("Raw Measure Index (from REAPER): {}\n", raw_measure).as_str(),
            );
            reaper.show_console_msg(
                format!(
                    "Adjusted Measure Index (with offset): {}\n",
                    adjusted_measure
                )
                .as_str(),
            );
...
fn log_project_measure_offset_handler() {
    ...
    if let Some(offs_result) = medium_reaper.project_config_var_get_offs("projmeasoffs") {
        // Get the actual value using the offset
        if let Some(addr) =
            medium_reaper.project_config_var_addr(current_project.context(), offs_result.offset)
        {
            // Read the integer value directly from the pointer (it's a 32-bit integer)
            let offset_value = unsafe { *(addr.as_ptr() as *const i32) };

            info!(
                project_measure_offset = offset_value,
                "Project measure offset read successfully"
            );
            ...
            reaper
                .show_console_msg(format!("Measure Correction: {}\n", measure_correction).as_str());
...
fn log_tempo_time_sig_changes_handler() {
    ...
    if markers.is_empty() {
        ...
        if changes.is_empty() {
            reaper.show_console_msg(
                "No tempo/time signature changes found (all markers have same values).\n",
            );
            info!("No tempo/time signature changes found");
        } else {
            reaper.show_console_msg(
                format!(
                    "Found {} tempo/time signature change(s):\n\n",
                    changes.len()
                )
                .as_str(),
            );
            info!(
                ...
                "Found {} tempo/time signature changes",
                changes.len()
            );
            ...
            for (idx, (time_pos, tempo, time_sig)) in changes.iter().enumerate() {
                ...
                let subdivision =
                    ((beats_since_measure - beats_since_measure.floor()) * 1000.0).round() as i32;
...
fn create_text_items_from_lyrics_handler() {
    fts::lyrics::infra::reaper::write::create_text_items_from_lyrics("".to_string())
        .unwrap_or_else(|e| {

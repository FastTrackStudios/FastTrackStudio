...
use crate::chart::{read, detection, build};
...
fn log_chords_handler() {
    ...
    info!(note_count = midi_notes.len(), "Read MIDI notes from CHORDS track");
    ...
    let detected_chords = detection::detect_chords_from_reaper_midi_notes(&midi_notes, min_chord_duration_ppq);
    ...
    info!(chord_count = detected_chords.len(), "Detected chords from MIDI notes");
    ...
    let chart = build::build_chart_from_chords(
        detected_chords,
        ...
        for pattern_len in 1..=8.min(measures.len() - i) {
            ...
            if repeat_count > 1 {
                if let Some((best_len, best_repeats)) = best_pattern {
                    if repeat_count > best_repeats || (repeat_count == best_repeats && pattern_len > best_len) {
...
fn measures_equal(a: &[keyflow::chart::types::Measure], b: &[keyflow::chart::types::Measure]) -> bool {
    if a.len() != b.len() {
        return false;
    }
...
fn debug_midi_and_sections_handler() {
    ...
    let time_sig = keyflow::time::TimeSignature { numerator: 4, denominator: 4 };
    let beats_per_measure = time_sig.numerator as f64;
    ...
    debug_output.push_str(&format!("Total MIDI notes: {} | Detected chords: {}\n\n", midi_notes.len(), detected_chords.len()));
    
    // Group notes by start_ppq
    let mut notes_by_time: BTreeMap<i64, Vec<&read::MidiNote>> = BTreeMap::new();
    for note in &midi_notes {
        notes_by_time.entry(note.start_ppq).or_insert_with(Vec::new).push(note);
    }
    ...
    for (start_ppq, notes) in notes_by_time.iter().take(30) {
        ...
        let mut notes_with_pitch: Vec<(u8, String)> = notes.iter()
            .map(|n| (n.pitch, midi_pitch_to_note_name(n.pitch)))
            .collect();
        notes_with_pitch.sort_by_key(|(pitch, _)| *pitch);
        let sorted_notes: Vec<String> = notes_with_pitch.iter()
        ...
        let musical_pos = MusicalPosition::try_new(measures, beats_in_measure, subdivision.clamp(0, 999))
        ...
        let musical_dur = MusicalDuration::try_new(dur_measures, dur_beats_in_measure, dur_subdivision.clamp(0, 999))
        ...
        if let Some(chord) = chord_by_start.get(start_ppq) {
            ...
            let chord_musical_pos = MusicalPosition::try_new(chord_measures, chord_beats_in_measure, chord_subdivision.clamp(0, 999))
                .unwrap_or_else(|_| MusicalPosition::start());
            ...
            let chord_musical_dur = MusicalDuration::try_new(chord_dur_measures, chord_dur_beats_in_measure, chord_dur_subdivision.clamp(0, 999))
                .unwrap_or_else(|_| MusicalDuration::zero());
    ...
    if notes_by_time.len() > 30 {
        debug_output.push_str(&format!("... and {} more note groups\n\n", notes_by_time.len() - 30));
    }
    ...
    if let Ok(setlist) = build_setlist_from_open_projects(None) {
        if let Some(name) = project_name {
            if let Some(song) = setlist.songs.iter().find(|s| {
                s.name == name || s.name.contains(&name) || name.contains(&s.name)
            }) {
                debug_output.push_str(&format!("Song: {} | {} sections\n\n", song.name, song.sections.len()));
                ...
                for (idx, section) in song.sections.iter().enumerate() {
                    ...
                    let notes_in_section: Vec<_> = midi_notes.iter()
                        .filter(|note| {
                    ...
                    let chords_in_section: Vec<_> = detected_chords.iter()
                        .filter(|chord| {
                    ...
                    let mut section_notes_by_time: BTreeMap<i64, Vec<&read::MidiNote>> = BTreeMap::new();
                    for note in &notes_in_section {
                        section_notes_by_time.entry(note.start_ppq).or_insert_with(Vec::new).push(note);
                    }
                    ...
                    for (start_ppq, notes) in section_notes_by_time.iter().take(8) {
                        ...
                        let mut notes_with_pitch: Vec<(u8, String)> = notes.iter()
                            ...
                            .collect();
                        notes_with_pitch.sort_by_key(|(pitch, _)| *pitch);
                        let sorted_note_names: Vec<String> = notes_with_pitch.iter()
                            .map(|(_, name)| name.clone())
                            .collect();
                        ...
                        let note_musical_pos = MusicalPosition::try_new(note_measures, note_beats, note_subdivision.clamp(0, 999))
                            .unwrap_or_else(|_| MusicalPosition::start());

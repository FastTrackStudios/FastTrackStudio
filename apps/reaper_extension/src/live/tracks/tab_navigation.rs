...
impl TabNavigator {
    ...
    fn find_next_valid_tab(start_from: usize, direction: i32) -> Option<usize> {
        ...
        while checked < tab_count {
            ...
            if direction > 0 {
                ...
                current = if current > 0 { current - 1 } else { tab_count - 1 };
            }
    ...
    pub fn get_tab_count(&mut self) -> usize {
        ...
        trace!(tab_count = count, excluded_tabs = excluded_count, "Tab count updated");
        count
    }
    ...
    pub fn get_current_tab_index(&self) -> Option<usize> {
        ...
        if let Some(current_result) = medium_reaper.enum_projects(ProjectRef::Current, 0) {
            ...
            let current_name = current_result.file_path
                .as_ref()
    ...
    pub fn switch_next(&self) -> anyhow::Result<()> {
        ...
        let current_tab = self.get_current_tab_index()
        ...
        let next_tab_info = if let Some(result) = medium_reaper.enum_projects(ProjectRef::Tab(next_tab as u32), 0) {
            result.file_path
                .as_ref()
                .and_then(|p| p.as_std_path().file_stem())
                .and_then(|s| s.to_str())
    ...
    pub fn switch_prev(&self) -> anyhow::Result<()> {
        ...
        // Get tab info for logging
        let prev_tab_info = if let Some(result) = medium_reaper.enum_projects(ProjectRef::Tab(prev_tab as u32), 0) {
            result.file_path
                .as_ref()
                .and_then(|p| p.as_std_path().file_stem())
                .and_then(|s| s.to_str())
    ...
    pub fn switch_to_tab(&self, tab_index: usize) -> anyhow::Result<()> {
        ...
        let tab_index_u32 = tab_index.try_into().map_err(|_| anyhow::anyhow!("Tab index {} too large", tab_index))?;
        
        // Verify tab exists and get info
        let tab_result = medium_reaper.enum_projects(ProjectRef::Tab(tab_index_u32), 0)
        ...
        if Self::is_excluded_tab(file_path.as_deref()) {
            warn!(tab_index, "Attempted to switch to excluded tab (FTS-ROUTING)");
            return Err(anyhow::anyhow!("Cannot switch to excluded tab (FTS-ROUTING)"));
        }
        
        let tab_name = tab_result.file_path
            .as_ref()
            .and_then(|p| p.as_std_path().file_stem())
            .and_then(|s| s.to_str())
            .map(|s| s.to_string())
        ...
        let current_idx = self.get_current_tab_index()
            .ok_or_else(|| anyhow::anyhow!("No current tab"))?;

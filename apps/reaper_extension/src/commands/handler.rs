...
use reaper_medium::{
    ...
    AcceleratorKeyCode, AcceleratorBehavior,
};
use std::sync::OnceLock;
use crate::commands::state::{CommandState, KeyPress, Context, Mode};
use crate::commands::bindings::Bindings;
use crate::commands::matcher::{match_sequence, default_action_sequences};
...
static INTERCEPTION_ENABLED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(true);
...
impl CommandHandler {
    ...
    fn process_keypress(&mut self, key: AcceleratorKeyCode, behavior: &enumflags2::BitFlags<AcceleratorBehavior>, context: Context, context_name: &str) -> (bool, bool) {
        ...
        if let Some((command, completions)) = match_sequence(&state, bindings, sequences) {
            ...
            if !command.action_sequence.is_empty() {
                ...
                reaper.show_console_msg(format!("FTS-Commands: Key sequence '{}' executed in {}\n", state.key_sequence, context_name));
...
impl TranslateAccel for CommandHandler {
    fn call(&mut self, args: TranslateAccelArgs) -> TranslateAccelResult {
        ...
        let (should_eat, is_complete_command) = self.process_keypress(key, &behavior, context, &context_name);
...
impl CommandHandler {
    ...
    pub fn set_enabled(enabled: bool) {
        ...
        info!("FTS-Commands interception {}", if enabled { "enabled" } else { "disabled" });
    }
    ...
    pub fn set_passthrough(enabled: bool) {
        ...
        info!("FTS-Commands passthrough mode {}", if enabled { "enabled" } else { "disabled" });
    }

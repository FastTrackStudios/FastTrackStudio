        ...
        if note_count > 0 {
            ...
            for note_idx in 0..(note_count as usize) {
                ...
                if success {
                    ...
                    let end_time = unsafe {
                        medium_reaper.low().MIDI_GetProjTimeFromPPQPos(take.raw().as_ptr(), end_ppq)
                    };
                    
                    // Convert PPQ to quarter notes, then to measure/beat/tick
                    let start_qn = unsafe {
                        medium_reaper.low().MIDI_GetProjQNFromPPQPos(take.raw().as_ptr(), start_ppq)
                    };
                    let end_qn = unsafe {
                        medium_reaper.low().MIDI_GetProjQNFromPPQPos(take.raw().as_ptr(), end_ppq)
                    };
                    ...
                    warn!(item_index = item_idx, note_index = note_idx, "Failed to get MIDI note");
                }
            }
        }
...
unsafe fn ppq_to_musical_position(
    ...
    let measure_result = medium_reaper.time_map_qn_to_measures(
        project,
    ...
    let measure_start_ppq = medium_reaper.low().MIDI_GetPPQPosFromProjQN(take.as_ptr(), measure_start_qn);
...
fn assign_syllables_to_notes_handler() {
    ...
    reaper.show_console_msg(ReaperStringArg::from(msg.as_str()));
    info!(total_assigned = total_notes_assigned, "Finished assigning syllables");
}

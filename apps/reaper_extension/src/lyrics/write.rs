...
pub fn create_text_items_from_lyrics(lyrics_text: String) -> anyhow::Result<()> {
    ...
    // Parse lyrics
    let lyrics = match lyrics::parse_lyrics(&lyrics_text, "Untitled Song".to_string()) {
        Ok(lyrics) => lyrics,
        Err(e) => {
    ...
    let tempo_bpm = {
        ...
        let marker_count = medium_reaper.count_tempo_time_sig_markers(current_project.context()) as i32;
    ...
    let (time_sig_num, time_sig_denom) = {
        let zero_pos = PositionInSeconds::new(start_position)
            .unwrap_or(PositionInSeconds::ZERO);
    ...
    for (idx, slide) in slides.iter().enumerate() {
        ...
        let item = unsafe {
            medium_reaper.low().AddMediaItemToTrack(selected_track.as_ptr())
        };
    ...
    unsafe {
        ...
        medium_reaper.low().SelectAllMediaItems(current_project.context().to_raw(), false);
...
pub fn update_lyrics_in_reaper(song_index: usize, lyrics: Lyrics) -> Result<(), String> {
    ...
    let start_position = song.start_marker
        .as_ref()
        .map(|m| m.position.time.to_seconds())
        .unwrap_or(0.0);
    
    let tempo_bpm = song.starting_tempo.unwrap_or_else(|| medium_reaper.master_get_tempo().get());
    let (time_sig_num, time_sig_denom) = song.starting_time_signature
    ...
    let track_raw = slides_track.raw().map_err(|e| format!("Failed to get track raw pointer: {}", e))?;
    for item in existing_items {
        unsafe {
            let item_raw = item.raw();
            medium_reaper.low().DeleteTrackMediaItem(track_raw.as_ptr(), item_raw.as_ptr());
        }
    }
    ...
    for (idx, slide) in slides.iter().enumerate() {
        // Create empty media item
        let item = unsafe {
            medium_reaper.low().AddMediaItemToTrack(track_raw.as_ptr())
        };

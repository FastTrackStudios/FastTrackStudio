...
pub fn init_reactive_stream_logger(rx: &ControlSurfaceRx) {
    ...
    rx.track_volume_changed().subscribe(move |track| {
        if track.is_available() {
            ...
            let db = volume.to_db();
            debug!(
                "🔊 TRACK VOLUME CHANGED: track={:?} volume={}",
                track.name(),
                db
            );
        } else {
    ...
    rx.track_pan_touched()
        .subscribe(move |(track, old_pan, new_pan)| {
            trace!(
                "👆 TRACK PAN TOUCHED: track={:?} old={:?} new={:?}",
                track, old_pan, new_pan
            );
        });
    ...
    rx.track_mute_changed().subscribe(move |track| {
        if track.is_available() {
            ...
            debug!(
                "🔇 TRACK MUTE CHANGED: track={:?} muted={}",
                track.name(),
                is_muted
            );
        } else {
    ...
    rx.track_arm_changed().subscribe(move |track| {
        if track.is_available() {
            ...
            debug!(
                "🎙️ TRACK ARM CHANGED: track={:?} armed={}",
                track.name(),
                is_armed
            );
        } else {
    ...
    rx.track_selected_changed()
        .subscribe(move |(track, is_selected)| {
            debug!(
                "✅ TRACK SELECTED CHANGED: track={:?} selected={}",
                track, is_selected
            );
        });
    ...
    rx.track_input_changed().subscribe(move |track| {
        if track.is_available() {
            ...
            trace!(
                "🎤 TRACK INPUT CHANGED: track={:?} input={:?}",
                track.name(),
                rec_input
            );
        } else {
    ...
    rx.track_input_monitoring_changed().subscribe(move |track| {
        if track.is_available() {
            ...
            trace!(
                "👂 TRACK INPUT MONITORING CHANGED: track={:?} mode={:?}",
                track.name(),
                monitoring
            );
        } else {
            trace!(
                "👂 TRACK INPUT MONITORING CHANGED: {:?} (unavailable)",
                track
            );
        }
    });
    ...
    rx.track_automation_mode_changed().subscribe(move |track| {
        if track.is_available() {
            ...
            trace!(
                "🤖 TRACK AUTOMATION MODE CHANGED: track={:?} mode={:?}",
                track.name(),
                auto_mode
            );
        } else {
            trace!(
                "🤖 TRACK AUTOMATION MODE CHANGED: {:?} (unavailable)",
                track
            );
        }
    });
    ...
    rx.hardware_output_send_count_changed()
        .subscribe(move |track| {
    ...
    rx.fx_focused().subscribe(move |fx_opt| match fx_opt {
    ...
    rx.repeat_state_changed().subscribe(move |_| {
        ...
        debug!(
            ...
            if is_repeat { "enabled" } else { "disabled" }
        );
    });

...
use swell_ui::{
    Menu,
    menu_tree::{Entry, anonymous_menu, item, menu, separator},
};
...
fn extension_menu() -> swell_ui::menu_tree::Menu<String> {
    ...
    debug!(
        menu_action_count = menu_actions.len(),
        "Actions to include in menu"
    );
    ...
    let mut categorized: HashMap<
        String,
        HashMap<Option<String>, Vec<&crate::infrastructure::action_registry::ActionDef>>,
    > = HashMap::new();
        ...
        if let Some(subcategories) = categorized.remove(*category_name) {
            if !subcategories.is_empty() {
                ...
                for (subcategory_opt, actions_in_subcategory) in subcategories {
                    if !actions_in_subcategory.is_empty() {
                        ...
                        for action in actions_in_subcategory {
                            let entry =
                                item(action.display_name.clone(), action.command_id.to_string());
                        ...
                        subcategory_items.sort_by(|a, b| match (a, b) {
                ...
                if !category_menu_entries.is_empty() {
                    categorized_menu_entries
                        .push(menu(category_name.to_string(), category_menu_entries));
                }
            }
        }
    }
    ...
    if let Some(general_subcategories) = categorized.remove("General") {
        ...
        for (_, general_actions) in general_subcategories {
            for action in general_actions {
                let entry = item(action.display_name.clone(), action.command_id.to_string());

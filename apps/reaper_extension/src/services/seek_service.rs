...
use reaper_medium::{
    MeasureMode, PositionInBeats, PositionInSeconds, ProjectRef, SetEditCurPosOptions,
};
use std::sync::mpsc;
...
impl SeekService {
        ...
        if let Ok(guard) = self.seek_rx.lock() {
            if let Some(receiver) = guard.as_ref() {
                ...
                while let Ok(request) = receiver.try_recv() {
                    let result =
                        self.seek_to_section_main_thread(request.song_index, request.section_index);
    ...
    fn seek_to_section_main_thread(
        &self,
        song_index: usize,
        section_index: usize,
    ) -> Result<(), String> {
        ...
        #[cfg(feature = "live")]
        {
            ...
            let musical_pos_to_time =
                |project: &Project,
                 ...
                 -> Result<PositionInSeconds, String> {
                    ...
                    let beats_since_measure =
                        musical_pos.beat as f64 + (musical_pos.subdivision as f64 / 1000.0);
                    ...
                    Ok(medium_reaper.time_map_2_beats_to_time(
                        project_context,
                        measure_mode,
                        beats_pos,
                    ))
            ...
            let target_pos = if is_same_song {
                section
                    ...
                    .and_then(|pos| {
                        musical_pos_to_time(&current_project, &pos.musical).map_err(|e| {
                            format!("Failed to convert musical position to time: {}", e)
                        })
                    })
            ...
            if is_playing && is_same_song {
                ...
                current_project.set_edit_cursor_position(
                    target_pos,
            ...
            let final_project = if !is_same_song {
                ...
                let target_pos = section
                    .start_position
                    .as_ref()
                    .ok_or_else(|| "Section has no start position".to_string())
                    .and_then(|pos| {
                        musical_pos_to_time(&project, &pos.musical).map_err(|e| {
                            format!("Failed to convert musical position to time: {}", e)
                        })
                    })
                    .map_err(|e| format!("Failed to get section start position: {}", e))?;

...
impl TriggerScheduler {
    ...
    /// - triggers: Vector of (sample_offset, priority) tuples
    ///   Priority: 0 = Beat (highest), 1 = Eighth, 2 = Sixteenth, 3 = Triplet (lowest)
    /// - The last_triggered values are updated based on current position and returned for use in next iteration
    #[allow(clippy::too_many_arguments)]
    pub fn schedule_subdivision_triggers(
        params: &TriggerSchedulingParams,
        intervals: &SubdivisionIntervals,
        _last_triggered_beat: f64,
        mut last_triggered_eighth: f64,
        mut last_triggered_sixteenth: f64,
        mut last_triggered_triplet: f64,
    ...
    /// Returns a map of sample -> priority (lowest number = highest priority)

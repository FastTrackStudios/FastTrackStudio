...
impl ServerContext {
    ...
    fn glossary_service(&self) -> task_core::service_impl::GlossaryServiceImpl {
        task_core::service_impl::GlossaryServiceImpl::new(
            task_core::service_impl::GlossaryServiceDeps {
                db: self.db.clone(),
            },
        )
    }
    ...
    fn nutrition_service(&self) -> task_core::service_impl::NutritionServiceImpl {
        task_core::service_impl::NutritionServiceImpl::new(
            task_core::service_impl::NutritionServiceDeps {
                db: self.db.clone(),
                openfoodfacts: self.openfoodfacts.clone(),
            },
        )
    ...
    let factory = vox::acceptor_fn(
        move |request: &vox::ConnectionRequest,
            ...
            match request.service() {
                ...
                "WorkoutSessionRepo" => {
                    connection.handle_with(
                        task_core::workout_session::WorkoutSessionRepoDispatcher::new(
                            task_core::workout_session::WorkoutSessionRepoStorage::new(db.clone()),
                        ),
                    );
                    Ok(())
                }
                "SetLogRepo" => {
                    connection.handle_with(task_core::set_log::SetLogRepoDispatcher::new(
                        task_core::set_log::SetLogRepoStorage::new(db.clone()),
                    ));
                    Ok(())
                }
                "FoodService" => {
                ...
                "GlossaryService" => {
                    connection.handle_with(task_core::GlossaryServiceDispatcher::new(
                        ctx.glossary_service(),
                    ));
                    Ok(())
                }
                "GlossaryTermRepo" => {
                    connection.handle_with(task_core::glossary::GlossaryTermRepoDispatcher::new(
                        task_core::glossary::GlossaryTermRepoStorage::new(db.clone()),
                    ));
                    Ok(())
                }
                "PantryService" => {
                ...
                "NutritionService" => {
                    connection.handle_with(task_core::service::NutritionServiceDispatcher::new(
                        ctx.nutrition_service(),
                    ));
                    Ok(())
                }
                "FoodLogRepo" => {
                    connection.handle_with(task_core::food_log::FoodLogRepoDispatcher::new(
                        task_core::food_log::FoodLogRepoStorage::new(db.clone()),
                    ));
                    Ok(())
                }
                "FoodRepo" => {
    ...
    async fn capabilities(&self) -> SystemCapabilities {
        SystemCapabilities {
            ...
            services: vec![
                ...
                "GlossaryTermRepo".into(),
                "FoodProductRepo".into(),
                "PantryService".into(),
                "PantryItemRepo".into(),
                "NutritionService".into(),
                "FoodLogRepo".into(),
                "RecipeRepo".into(),

...
async fn handle_vox_connection(socket: WebSocket, state: AppState) {
    ...
    // then wrap in vox-websocket WsLink
    let svc = state.svc.clone();

    // Use the Vox service descriptor to serve requests.
    // The service descriptor is generated by #[vox::service] on VaultService.
    let descriptor = vault_service_service_descriptor();

    // For now, we handle the WebSocket connection by logging it.
    // Full Vox serving requires wiring the WsLink into vox-core's serve loop.
    // TODO: wire vox_core::serve(WsLink::new(stream), svc, descriptor)
    // This requires converting axum's WebSocket type to a tokio-tungstenite
    // WebSocketStream, which needs the axum "ws" feature's underlying stream.

    // Placeholder: keep connection alive, respond to pings
    ...
    while let Some(Ok(msg)) = rx.next().await {
        match msg {
            ...
            _ => {
                // Binary/text messages will be Vox RPC frames once fully wired
            }
        }
    }

...
impl SceneNode {
    ...
    /// # Example
    /// ```ignore
    /// use keyflow::{TextSpan, ChartPosition};
    ...
    pub fn set_section_type(&mut self, section_type: &str) -> &mut Self {
        self.metadata
            .insert(metadata_keys::SECTION_TYPE.to_string(), section_type.to_string());
        self
    }
    ...
    pub fn get_section_type(&self) -> Option<&str> {
        self.metadata.get(metadata_keys::SECTION_TYPE).map(String::as_str)
    }

    /// Set the element type for this node.
    pub fn set_element_type(&mut self, element_type: &str) -> &mut Self {
        self.metadata
            .insert(metadata_keys::ELEMENT_TYPE.to_string(), element_type.to_string());
        self
    }
    ...
    pub fn get_element_type(&self) -> Option<&str> {
        self.metadata.get(metadata_keys::ELEMENT_TYPE).map(String::as_str)
    }

    /// Set the font family for this node.
    pub fn set_font_family(&mut self, font_family: &str) -> &mut Self {
        self.metadata
            .insert(metadata_keys::FONT_FAMILY.to_string(), font_family.to_string());
        self
    }
    ...
    pub fn get_font_family(&self) -> Option<&str> {
        self.metadata.get(metadata_keys::FONT_FAMILY).map(String::as_str)
    }
    ...
    pub fn is_smufl_glyph(&self) -> bool {
        self.get_glyph_info()
            .map_or(false, |info| info.is_smufl())
    }
    ...
    pub fn is_text_glyph(&self) -> bool {
        self.get_glyph_info()
            .map_or(false, |info| info.is_text())
    }
    ...
    #[must_use]
    pub fn build_chart_index(&self) -> keyflow::ChartIndex {
        let mut index = keyflow::ChartIndex::new();
    ...
    #[cfg(feature = "keyflow-import")]
    fn build_chart_index_recursive(&self, index: &mut keyflow::ChartIndex, counter: &mut u64) {
        // Try to get source link from metadata
        if let Some(source_link) =
            self.get_json_metadata::<keyflow::SourceLink>(metadata_keys::SOURCE_LINK)
        {
            // Generate element ID
            let element_id = self
                .id
            ...
            if let Some(position) =
                self.get_json_metadata::<keyflow::ChartPosition>(metadata_keys::CHART_POSITION)
            {
                let element_id = self
                    .id
                    .as_ref()
    ...
    pub fn find_by_source_offset(&self, offset: usize) -> Vec<&SceneNode> {
        self.find_all(|node| {
            node.get_json_metadata::<keyflow::TextSpan>(metadata_keys::SOURCE_SPAN)
    ...
    pub fn find_by_chart_position(&self, measure: u32, beat: u32) -> Vec<&SceneNode> {
        self.find_all(|node| {
            node.get_json_metadata::<keyflow::ChartPosition>(metadata_keys::CHART_POSITION)
...
mod tests {
    ...
    fn test_json_metadata() {
        ...
        let pos = TestPosition { measure: 4, beat: 2 };
    ...
    fn test_glyph_info_symbol() {
        let info = GlyphInfo::symbol('♯', "sans-serif")
            .with_description("Sharp sign");
    ...
    fn test_with_glyph_info_builder() {
        let glyph = GlyphInfo::smufl('\u{E050}', "Leland")
            .with_smufl_name("gClef");
    ...
    fn example_glyph_inventory() {
        ...
        for (idx, (codepoint, name, font)) in glyphs.iter().enumerate() {
            let mut node = SceneNode::new();
            let info = GlyphInfo::smufl(*codepoint, *font)
                .with_smufl_name(*name);
        ...
        let mut unique_codepoints: std::collections::HashSet<u32> = std::collections::HashSet::new();
        for node in &smufl_nodes {
    ...
    fn example_verify_font_assignments() {
        ...
        for (idx, section) in sections.iter().enumerate() {
            let mut label = SceneNode::group(SemanticId::rehearsal_mark(idx as u64 + 100, *section));
        ...
        for node in &chord_nodes {
            ...
            assert_eq!(info.font_family, "MuseJazz", "Chord symbols must use MuseJazz");
        ...
        for node in &section_nodes {
            let info = node.get_glyph_info().expect("Section label should have glyph info");
            assert_eq!(info.font_family, "Helvetica Bold", "Section labels must use Helvetica Bold");
        }
        ...
        for node in &lyric_nodes {
            ...
            assert_eq!(info.font_family, "Times New Roman", "Lyrics must use Times New Roman");
        }
    ...
    fn example_style_configured_fonts() {
        ...
        assert_eq!(
            jazz_chord.get_glyph_info().unwrap().font_family,
            "MuseJazz"
        );

        let jazz_lyric = jazz_scene.find_by_element_type("lyric")[0];
        assert_eq!(
            jazz_lyric.get_glyph_info().unwrap().font_family,
            "Georgia"
        );
        ...
        assert_eq!(classical_note.get_glyph_info().unwrap().font_family, "Bravura");
    }
}

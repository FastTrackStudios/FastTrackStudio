...
impl LayoutContextOwned {
    ...
    #[must_use]
    pub fn new(config: LayoutConfiguration, score: Score, style: MStyle, font: SMuFLFont<'static>) -> Self {
        Self {
            config,
            score,
            style,
...
impl<'a> LayoutContext<'a> {
    ...
    #[deprecated(since = "0.2.0", note = "Use LayoutContextOwned::new_minimal() instead")]
    #[must_use]
...
pub fn test_context() -> LayoutContext<'static> {
    let config = Box::leak(Box::new(LayoutConfiguration::new()));
    let score = Box::leak(Box::new(crate::model::Score::default()));
    let style = Box::leak(Box::new(crate::style::MStyle::default()));
    let font = Box::leak(Box::new(crate::fonts::SMuFLFont::empty()));
...
#[cfg(test)]
pub fn test_context_with_style(style: crate::style::MStyle) -> LayoutContext<'static> {
    let config = Box::leak(Box::new(LayoutConfiguration::new()));
    let score = Box::leak(Box::new(crate::model::Score::default()));

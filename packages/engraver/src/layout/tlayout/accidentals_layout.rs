...
impl<'a> LayoutContext<'a> {
    ...
    fn vertical_padding(&self, acc1_type: AccidentalType, acc1_mag: f64, acc2_type: AccidentalType, acc2_mag: f64) -> f64 {
...
pub fn layout_accidentals(
    ...
    if accidentals.len() == 1 {
        let x_offset =
            -(accidentals[0].width + config.accidental_to_note_distance) * spatium;
...
fn find_octaves_and_seconds(accidentals: &mut [AccidentalInfo]) {
    ...
    for i in 0..len {
        for j in (i + 1)..len {
            ...
            if line_diff % 7 == 0 && accidentals[i].accidental_type == accidentals[j].accidental_type
            {
...
fn insert_seconds(
    ...
    for second_id in second_ids {
        ...
        if let Some(pos) = remaining.iter().position(|&i| accidentals[i].id == second_id) {
...
fn insert_octave(
    ...
    for octave_id in octave_ids {
        if let Some(pos) = remaining.iter().position(|&i| accidentals[i].id == octave_id) {
...
fn can_fit_in_same_column(acc1: &AccidentalInfo, acc2: &AccidentalInfo, ctx: &LayoutContext) -> bool {
    // Exception: naturals a sixth apart can share a column
    if is_exception_of_naturals_sixth(acc1, acc2) {
        return true;
    }

    let v_pad = ctx.vertical_padding(acc1.accidental_type, acc1.mag, acc2.accidental_type, acc2.mag);
    ...
    loop {
        ...
        for stacked in &ctx.stacked {
            ...
            if is_exception_of_fourth_stacked(acc, stacked) {
                let kern = kerning_of_fourth_stacked(acc, stacked, &acc_shape, &stacked.shape, config);
            ...
            let v_pad = ctx.vertical_padding(acc.accidental_type, acc.mag, stacked.accidental_type, stacked.mag);
            ...
            if acc_shape.intersects(&stacked.shape, h_pad * 0.999, v_pad) {
                ...
                if acc_shape.vertically_intersects(&stacked.shape, v_pad) {
                    ...
                    let dist = dist + additional_padding_for_verticals_stacked(acc, stacked, config) * spatium;
...
fn min_accidental_to_chord_distance(acc: &AccidentalInfo, ctx: &LayoutContext) -> f64 {
    ...
    for (note_line, notehead_bbox) in &ctx.chord_shape.noteheads {
        ...
        if acc.shape.bottom + clearance >= notehead_top && acc.shape.top - clearance <= notehead_bottom
        {
...
fn vertically_align_accidentals(accidentals: &mut [AccidentalInfo], ctx: &LayoutContext) {
    ...
    for i in 0..accidentals.len() {
        ...
        for j in (i + 1)..accidentals.len() {
            ...
            let similar_x =
                (acc1.x_offset - acc2.x_offset).abs() < ctx.config.x_vertical_alignment_threshold * ctx.spatium;
...
fn is_exception_of_fourth(acc1: &AccidentalInfo, acc2: &AccidentalInfo) -> bool {
    ...
    let acc2_flat_or_nat = acc2.accidental_type.is_flat() || acc2.accidental_type == AccidentalType::Natural;
...
mod tests {
    ...
    fn test_two_accidentals_with_overlap() {
        ...
        assert!(placements[0].column != placements[1].column ||
    ...
    fn test_complex_chord() {
        ...
        for p in &placements {
            assert!(p.x_offset < 0.0, "Accidentals should be to the left of notes");
        }
    }

...
fn layout_articulation_at(
    ...
    let commands = vec![PaintCommand::glyph(glyph, position, spatium * config.scale, Color::BLACK)];
...
mod tests {
    ...
    fn test_articulation_glyphs() {
        assert_eq!(ArticulationType::Staccato.smufl_codepoint_above(), '\u{E4A2}');
        assert_eq!(ArticulationType::Staccato.smufl_codepoint_below(), '\u{E4A3}');
    ...
    fn test_stacking_priority() {
        ...
        assert!(ArticulationType::Staccato.stacking_priority() < ArticulationType::Accent.stacking_priority());
        assert!(ArticulationType::Tenuto.stacking_priority() < ArticulationType::Marcato.stacking_priority());
    }
    ...
    fn test_single_articulation_above() {
        ...
        assert_eq!(layout.glyph, ArticulationType::Staccato.smufl_codepoint_above());
    }
    ...
    fn test_single_articulation_below() {
        ...
        assert_eq!(layout.glyph, ArticulationType::Accent.smufl_codepoint_below());
    }
    ...
    fn test_stacking_multiple_articulations() {
        ...
        let Some(staccato) = layouts.iter().find(|l| l.articulation_type == ArticulationType::Staccato) else {
            panic!("Expected staccato articulation in results");
        };
        let Some(marcato) = layouts.iter().find(|l| l.articulation_type == ArticulationType::Marcato) else {

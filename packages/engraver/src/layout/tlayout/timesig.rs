...
pub fn layout_timesig(params: &TimeSigParams, ctx: &LayoutContext) -> (LayoutData, SceneNode) {
    ...
    match &params.sig_type {
        ...
        TimeSigType::Numeric { numerator, denominator } => {
            let (num_commands, num_width) = layout_number(*numerator, 0.0, -spatium, spatium * scale);
            let (denom_commands, denom_width) = layout_number(*denominator, 0.0, spatium, spatium * scale);
        ...
        TimeSigType::SingleNumber(n) => {
            ...
            let semantic_id = SemanticId::new(ElementType::TimeSignature, params.id);
            let node = SceneNode::leaf(semantic_id, commands)
                .with_metadata("timesig", n.to_string());

            return (layout, node);
        }

        TimeSigType::Additive { groups, denominator } => {
            ...
            for cmd in denom_commands {
                if let PaintCommand::Glyph { codepoint, position, size, color } = cmd {
                    commands.push(PaintCommand::glyph(
                        codepoint,
            ...
            let semantic_id = SemanticId::new(ElementType::TimeSignature, params.id);
            let node = SceneNode::leaf(semantic_id, commands)
                .with_metadata("timesig", format!("{}+/{}", groups_str.join("+"), denominator));

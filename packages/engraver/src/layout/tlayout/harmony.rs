...
impl HarmonyStyle {
    ...
    #[must_use]
    pub fn with_font_data(mut self, text_font_data: Arc<Vec<u8>>, symbol_font_data: Option<Arc<Vec<u8>>>) -> Self {
...
pub fn layout_harmony(
    ...
    // Helper to measure text width using font metrics or estimation
    // This closure captures the style's font metrics for use in layout
    let text_metrics = style.text_font_metrics.as_ref();
    let symbol_metrics = style.symbol_font_metrics.as_ref();

    // Measure text width using actual font metrics when available
    let measure_text_width = |text: &str, font_size: f64, use_symbol_font: bool| -> f64 {
        // Use appropriate font metrics if available
        let metrics = if use_symbol_font { symbol_metrics } else { text_metrics };
        if let Some(fm) = metrics {
            return fm.horizontal_advance(text, font_size);
        }

        // Fallback to estimation when no font metrics provided
        // These ratios are calibrated for FreeSans-like fonts
        text.chars()
            .map(|c| {
                let ratio = match c {
                    ...
                    'm' | 'w' | 'M' | 'W' => 0.9,
                    // Wide uppercase
                    'A' | 'B' | 'C' | 'D' | 'G' | 'H' | 'K' | 'N' | 'O' | 'Q' | 'R' | 'U' | 'V' | 'X' | 'Y' | 'Z' => 0.72,
                    'E' | 'F' | 'L' | 'P' | 'S' | 'T' => 0.62,
                    'I' | 'J' => 0.42,
                    // Lowercase by width
                    'a' | 'e' | 'o' | 'n' | 'u' | 'v' | 'x' | 'z' => 0.52,
                    'b' | 'd' | 'g' | 'h' | 'k' | 'p' | 'q' | 'y' => 0.55,
                    's' | 'c' => 0.48,
                    'r' => 0.36,
                    'i' | 'j' | 'l' => 0.28,
                    't' | 'f' => 0.32,
                    // Numbers (typically tabular)
                    '0'..='9' => 0.55,
                    // Punctuation
                    '/' => 0.35,
                    '(' | ')' => 0.35,
                    '+' | '-' | '=' => 0.55,
                    ' ' => 0.28,
                    // SMuFL chord symbols (from Leland font metrics, scaled)
                    '\u{E873}' => 0.9,  // csymMajorSeventh (△)
                    '\u{E870}' => 0.55, // csymDiminished (°)
                    '\u{E871}' => 0.6,  // csymHalfDiminished (ø)
                    '\u{E872}' => 0.55, // csymAugmented (+)
                    '\u{ED60}' | '\u{ED62}' => 0.5, // Accidentals
                    '\u{266D}' | '\u{266F}' | '\u{266E}' => 0.5,  // ♭♯♮
                    '\u{25B3}' | '\u{00B0}' | '\u{00F8}' => 0.55, // △°ø
                    _ if c as u32 > 0x100 => 0.7,
                    _ => 0.55,
                };
                font_size * ratio
            })
    ...
    // MuseScore spacing parameters (as fraction of font size)
    // MuseScore uses cap-height units (~0.7 of font size), but we use font size directly
    // m:0.036:0 in MuseScore = 0.036 * cap_height ≈ 0.025 * font_size
    // ms:0.1:0 in MuseScore = 0.1 * scaled_cap_height ≈ 0.07 * font_size
    // We use slightly larger values for better visual separation
    let space_after_root = 0.08;           // Space after root letter (m:0.036:0 scaled up)
    let space_around_accidental = 0.14;    // Padding around accidentals (ms:0.1:0)
    let space_after_quality = 0.08;        // Space after quality before extension
    let space_before_extension = 0.1;      // Gap before superscript elements
    let space_between_alterations = 0.06;  // Gap between alterations
    let space_before_slash = 0.1;          // Space before slash in slash chords

    // Font to use for symbol glyphs (accidentals, special chars)
    let symbol_font = style.symbol_font_family.as_ref().unwrap_or(&style.font_family);
    ...
    if !params.root_accidental.is_empty() {
        cursor_x += style.root_size * space_around_accidental; // Padding before accidental
        let acc_text = format_accidental(&params.root_accidental, style.symbol_set);
        let acc_width = measure_text_width(&acc_text, style.root_size, true); // Accidentals use symbol font
        commands.push(PaintCommand::text(
            acc_text,
            symbol_font,
            style.root_size,
        ...
        cursor_x += acc_width;
        cursor_x += style.root_size * space_around_accidental; // Padding after accidental
    }

    // Always add space after root (with or without accidental)
    cursor_x += style.root_size * space_after_root;
    ...
    if has_quality {
        ...
        let quality_font = if is_symbol { symbol_font } else { &style.font_family };
        ...
        // Add spacing after quality
        cursor_x += style.root_size * space_after_quality;
    }
    ...
    // Add spacing before superscript elements if there's no quality
    if (has_extension || has_alterations) && !has_quality {
        cursor_x += style.root_size * space_before_extension;
    }
    ...
    if has_extension {
        ...
        let ext_y = baseline_y + style.root_size * style.superscript_offset;
        let ext_font = if has_symbols { symbol_font } else { &style.font_family };
        commands.push(PaintCommand::text(
            ext_text,
            ext_font,
            ext_size,
        ...
        cursor_x += ext_width;
        // Add spacing after extension if there are alterations
        if has_alterations {
            cursor_x += ext_size * space_between_alterations;
        }
    }
    ...
    if has_alterations {
        ...
        let alt_y = baseline_y + style.root_size * style.superscript_offset;

        for (i, alt) in params.alterations.iter().enumerate() {
            // Add spacing between alterations (not before first)
            if i > 0 {
                cursor_x += alt_size * space_between_alterations;
            }
            let alt_text = format_alteration(alt, style.symbol_set);
            let alt_width = measure_text_width(&alt_text, alt_size, true); // Alterations use symbol font
            commands.push(PaintCommand::text(
                alt_text,
                symbol_font,
                alt_size,
    ...
    if let Some(bass) = &params.bass {
        // Add spacing before slash
        cursor_x += style.root_size * space_before_slash;
        let bass_size = style.root_size * style.bass_scale;
        let slash_width = measure_text_width("/", bass_size, false); // Slash uses text font
        ...
        // Small space after slash
        cursor_x += bass_size * 0.03;

        // Bass note letter
        let bass_width = measure_text_width(bass, bass_size, false); // Bass letter uses text font
        commands.push(PaintCommand::text(
            bass.clone(),
            &style.font_family,
            bass_size,
        ...
        if !params.bass_accidental.is_empty() {
            // Small space before accidental
            cursor_x += bass_size * space_around_accidental * 0.5;
            let bass_acc_text = format_accidental(&params.bass_accidental, style.symbol_set);
            let bass_acc_width = measure_text_width(&bass_acc_text, bass_size, true); // Accidentals use symbol font
            commands.push(PaintCommand::text(
                bass_acc_text,
                symbol_font,
                bass_size,
    ...
    // The transform positions the node in world space
...
fn format_extension(ext: &str, notation: ChordNotation, symbol_set: SymbolSet) -> String {
    match (ext, notation, symbol_set) {
        ...
        ("Maj7", ChordNotation::Jazz, SymbolSet::Unicode) => format!("{}7", unicode_fallback::TRIANGLE),
        ("Maj9", ChordNotation::Jazz, SymbolSet::Unicode) => format!("{}9", unicode_fallback::TRIANGLE),
        ("Maj13", ChordNotation::Jazz, SymbolSet::Unicode) => format!("{}13", unicode_fallback::TRIANGLE),
        ...
        ("7b5", ChordNotation::Jazz, SymbolSet::Unicode) => format!("{}7", unicode_fallback::OSLASH),
        ...
        if s.starts_with(pattern) {
            ...
            if !remaining.is_empty() {
                ...
                while !alt_str.is_empty() {
                    if alt_str.starts_with("b5") || alt_str.starts_with("#5") ||
                       alt_str.starts_with("b9") || alt_str.starts_with("#9") ||
                       alt_str.starts_with("b13") || alt_str.starts_with("#13") ||
                       alt_str.starts_with("b11") || alt_str.starts_with("#11") {
                        let alt = if alt_str.len() >= 3 && alt_str.chars().nth(2).map_or(false, |c| c.is_ascii_digit()) {
                            alt_str[..3].to_string()
                        } else {
                            alt_str[..2].to_string()
                        };
...
mod tests {
    ...
    fn test_layout_harmony_basic() {
        let ctx = make_ctx();
        let params = HarmonyParams::major("C").at(100.0, 50.0);
    ...
    fn test_format_quality_jazz_unicode() {
        ...
        assert_eq!(format_quality("dim", ChordNotation::Jazz, SymbolSet::Unicode), "\u{00B0}");
        assert_eq!(format_quality("aug", ChordNotation::Jazz, SymbolSet::Unicode), "+");
    }

    #[test]
    fn test_format_quality_jazz_smufl() {
        // Test SMuFL symbols
        assert_eq!(format_quality("dim", ChordNotation::Jazz, SymbolSet::Smufl), "\u{E870}");
        assert_eq!(format_quality("aug", ChordNotation::Jazz, SymbolSet::Smufl), "\u{E872}");
    }

    #[test]
    fn test_format_quality_jazz_musejazz() {
        // Test MuseJazz PUA symbols
        assert_eq!(format_quality("dim", ChordNotation::Jazz, SymbolSet::MuseJazz), "\u{E18E}");
        assert_eq!(format_quality("aug", ChordNotation::Jazz, SymbolSet::MuseJazz), "\u{E186}");
    }

    #[test]
    fn test_format_extension_jazz_unicode() {
        // Test Unicode fallback
        assert_eq!(format_extension("Maj7", ChordNotation::Jazz, SymbolSet::Unicode), "\u{25B3}7");
    }

    #[test]
    fn test_format_extension_jazz_smufl() {
        // Test SMuFL symbols
        assert_eq!(format_extension("Maj7", ChordNotation::Jazz, SymbolSet::Smufl), "\u{E873}7");
    }

    #[test]
    fn test_format_extension_jazz_musejazz() {
        // Test MuseJazz PUA symbols
        assert_eq!(format_extension("Maj7", ChordNotation::Jazz, SymbolSet::MuseJazz), "\u{E18A}7");
    }
}

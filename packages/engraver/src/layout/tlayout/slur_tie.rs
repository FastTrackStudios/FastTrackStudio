...
pub fn layout_slur_with_obstacles(
    ...
    layout_curve(start, end, direction, ElementType::Slur, id, spatium, config, obstacles)
}
...
pub fn layout_tie_with_obstacles(
    ...
    layout_curve(start, end, direction, ElementType::Tie, id, spatium, config, obstacles)
}
...
fn layout_curve(
    ...
    let (mut cp1, mut cp2) = compute_initial_control_points(
        p1, p2, length, length_sp, spatium, is_up, config
    );
...
fn avoid_collisions(
    ...
    for iter in 0..config.max_collision_iterations {
        ...
        for (i, rect) in slur_rects.iter().enumerate() {
            ...
            if (in_left && collision.left) || (in_mid && collision.mid) || (in_right && collision.right) {
                continue;
            }
...
mod tests {
    ...
    fn test_slur_collision_avoidance_no_obstacles() {
        ...
        let result_with = layout_slur_with_obstacles(
            &start, &end, SlurDirection::Up, &[], 1, 5.0, &config
        );

        // Control points should be the same
        assert!((result_without.control_points.cp1.y - result_with.control_points.cp1.y).abs() < 0.01);
    }
    ...
    fn test_slur_collision_avoidance_with_obstacle() {
        ...
        let result_with = layout_slur_with_obstacles(
            &start, &end, SlurDirection::Up, &[obstacle], 1, spatium, &config
        );
    ...
    fn test_slur_collision_left_obstacle() {
        ...
        let result = layout_slur_with_obstacles(
            &start, &end, SlurDirection::Up, &[obstacle], 1, 5.0, &config
        );
    ...
    fn test_slur_collision_down_direction() {
        ...
        let result_with = layout_slur_with_obstacles(
            &start, &end, SlurDirection::Down, &[obstacle], 1, spatium, &config
        );
    ...
    fn test_slur_collision_multiple_obstacles() {
        ...
        let result = layout_slur_with_obstacles(
            &start, &end, SlurDirection::Up, &obstacles, 1, 5.0, &config
        );
    ...
    fn test_slur_obstacle_at_endpoints() {
        ...
        let result = layout_slur_with_obstacles(
            &start, &end, SlurDirection::Up, &[obstacle_start, obstacle_end], 1, 5.0, &config
        );

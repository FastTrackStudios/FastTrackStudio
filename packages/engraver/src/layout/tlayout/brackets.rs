...
fn layout_brace_glyph(
    ...
    // Create scene node
    let mut scene = SceneNode::group(SemanticId::new(ElementType::Custom, input.id)
...
fn layout_brace_path(
    ...
    let commands = vec![PaintCommand::filled_path(translated_path.clone(), config.color)];
...
fn layout_square_bracket(
    ...
    let bbox = Rect::new(x, top_y - line_width / 2.0, x + hook_width, bottom_y + line_width / 2.0);

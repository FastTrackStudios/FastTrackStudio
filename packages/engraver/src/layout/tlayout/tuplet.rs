...
use crate::scene::paint::{FontStyle, FontWeight, TextAnchor};
...
pub fn layout_tuplet(
    ...
    let min_y = p1.y.min(p2.y).min(p1.y + hook_height).min(p2.y + hook_height);
    let max_y = p1.y.max(p2.y).max(p1.y + hook_height).max(p2.y + hook_height);
...
fn calculate_bracket_endpoints(
    ...
    let (y1, y2) = if is_up {
        // Bracket goes above
        let y1 = if let Some(stem_tip) = first.y_stem_tip {
            if first.stem_direction == StemDirection::Up ||
               (first.stem_direction == StemDirection::Auto && stem_tip < first.y_head) {
                stem_tip - stem_offset
            } else {
                first.y_head - head_offset
            }
        } else {
            first.y_head - head_offset
        };

        let y2 = if let Some(stem_tip) = last.y_stem_tip {
            if last.stem_direction == StemDirection::Up ||
               (last.stem_direction == StemDirection::Auto && stem_tip < last.y_head) {
                stem_tip - stem_offset
            } else {
                last.y_head - head_offset
            }
        } else {
            last.y_head - head_offset
        };
        ...
        // Bracket goes below
        let y1 = if let Some(stem_tip) = first.y_stem_tip {
            if first.stem_direction == StemDirection::Down ||
               (first.stem_direction == StemDirection::Auto && stem_tip > first.y_head) {
                stem_tip + stem_offset
            } else {
                first.y_head + head_offset
        ...
        let y2 = if let Some(stem_tip) = last.y_stem_tip {
            if last.stem_direction == StemDirection::Down ||
               (last.stem_direction == StemDirection::Auto && stem_tip > last.y_head) {
                stem_tip + stem_offset
            } else {
                last.y_head + head_offset
...
/// Avoid collisions with inner notes.
fn avoid_collisions(
    notes: &[TupletNote],
    mut p1: Point,
    mut p2: Point,
    ...
    // Check each inner note for collision
    for note in notes.iter().skip(1).take(notes.len() - 2) {
        ...
        // Check for collision
        let margin = config.stem_distance * spatium;
        if is_up {

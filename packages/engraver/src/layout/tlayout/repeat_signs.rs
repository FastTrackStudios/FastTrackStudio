...
impl JumpType {
    ...
    pub const fn jump_to(self) -> &'static str {
        match self {
            Self::DaCapo | Self::DaCapoAlFine | Self::DaCapoAlCoda | Self::DaCapoAlDblCoda => "start",
            Self::DalSegno | Self::DalSegnoAlFine | Self::DalSegnoAlCoda | Self::DalSegnoAlDblCoda => "segno",
    ...
    pub const fn continue_at(self) -> &'static str {
        match self {
            Self::DaCapoAlCoda | Self::DalSegnoAlCoda
            | Self::DaCapoAlDblCoda | Self::DalSegnoAlDblCoda => "codab",
            _ => "",
        }
    }
...
pub fn layout_marker(
    ...
    let y = match input.placement {
        RepeatPlacement::Above => {
            input.staff_top - config.min_distance * spatium - height / 2.0 + config.y_offset * spatium
        }
        RepeatPlacement::Below => {
            input.staff_bottom + config.min_distance * spatium + height / 2.0 + config.y_offset * spatium
        }
    };
...
pub fn layout_jump(
    ...
    let y = match input.placement {
        RepeatPlacement::Above => {
            input.staff_top - config.min_distance * spatium - text_height / 2.0 + config.y_offset * spatium
        }
        RepeatPlacement::Below => {
            input.staff_bottom + config.min_distance * spatium + text_height / 2.0 + config.y_offset * spatium
        }
    };
...
mod tests {
    ...
    fn test_layout_coda_marker() {
        let input = MarkerInput::new(MarkerType::Coda, 100.0, 0.0, 20.0)
            .with_id(2);
        let config = RepeatSignConfig::default();
        let result = layout_marker(&input, 5.0, &config);
    ...
    fn test_layout_fine_marker() {
        let input = MarkerInput::new(MarkerType::Fine, 100.0, 0.0, 20.0)
            .with_id(3);
        let config = RepeatSignConfig::default();
        let result = layout_marker(&input, 5.0, &config);
    ...
    fn test_layout_codetta_marker() {
        let input = MarkerInput::new(MarkerType::Codetta, 100.0, 0.0, 20.0)
            .with_id(4);
        let config = RepeatSignConfig::default();
        let result = layout_marker(&input, 5.0, &config);
    ...
    fn test_layout_dc_jump() {
        let input = JumpInput::new(JumpType::DaCapo, 100.0, 0.0, 20.0)
            .with_id(1);
        let config = RepeatSignConfig::default();
    ...
    fn test_layout_ds_al_coda_jump() {
        let input = JumpInput::new(JumpType::DalSegnoAlCoda, 100.0, 0.0, 20.0)
            .with_id(2);
        let config = RepeatSignConfig::default();
        let result = layout_jump(&input, 5.0, &config);
    ...
    fn test_layout_to_coda_with_symbol() {
        let input = JumpInput::new(JumpType::ToCodaSym, 100.0, 0.0, 20.0)
            .with_id(3);
        let config = RepeatSignConfig::default();
        let result = layout_jump(&input, 5.0, &config);

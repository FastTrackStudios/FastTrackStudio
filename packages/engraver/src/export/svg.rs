...
use crate::scene::id::SemanticId;
use crate::scene::node::SceneNode;
use crate::scene::paint::PaintCommand;
use crate::scene::transform::{get_translation, is_identity};
    ...
    fn write_paint_command(&mut self, cmd: &PaintCommand, _transform: Affine) {
        ...
        match cmd {
            ...
            PaintCommand::Rect {
                ...
                let mut attrs = format!(r#"x="{x}" y="{y}" width="{w}" height="{h}" fill="{fill_str}""#);
            ...
            PaintCommand::Ellipse {
                ...
                let fill_str = fill.map_or("none".to_string(), color_to_svg);
                let mut attrs = format!(r#"cx="{cx}" cy="{cy}" rx="{rx}" ry="{ry}" fill="{fill_str}""#);
...
fn color_to_svg(color: vello::peniko::Color) -> String {
    ...
    if rgba.a == 255 {
        ...
        format!("rgba({},{},{},{:.2})", rgba.r, rgba.g, rgba.b, rgba.a as f64 / 255.0)
    }
}
...
fn text_anchor_to_svg(anchor: crate::scene::paint::TextAnchor) -> &'static str {
    use crate::scene::paint::TextAnchor;
    match anchor {
...
fn font_weight_to_svg(weight: crate::scene::paint::FontWeight) -> String {
    use crate::scene::paint::FontWeight;
    match weight {
...
fn font_style_to_svg(style: crate::scene::paint::FontStyle) -> &'static str {
    use crate::scene::paint::FontStyle;
    match style {
...
mod tests {
    ...
    fn test_metadata_attributes() {
        ...
        let scene = SceneNode::leaf(
            SemanticId::new(ElementType::Clef, 1),
            vec![],
        )

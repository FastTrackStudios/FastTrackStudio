...
impl Canvas2D {
    ...
    pub fn fill_rect(&mut self, rect: Rect, color: Color) {
        ...
        self.vertices.extend_from_slice(&[
            ...
            Vertex2D { position: p3, color: c },
            Vertex2D { position: p4, color: c },
            Vertex2D { position: p2, color: c },
        ]);
    }

    /// Draw a line from (x1, y1) to (x2, y2)
    pub fn stroke_line(&mut self, x1: f32, y1: f32, x2: f32, y2: f32, thickness: f32, color: Color) {
        let dx = x2 - x1;
        let dy = y2 - y1;
    ...
    pub fn stroke_rounded_rect(&mut self, rect: Rect, radius: f32, thickness: f32, color: Color) {
        ...
        builder.arc_to(
            ...
            lyon::path::ArcFlags { large_arc: false, sweep: true },
            point(x + w, y + r),
        );
    ...
    pub fn fill_circle(&mut self, cx: f32, cy: f32, radius: f32, color: Color) {
        ...
        for i in 0..segments {
            ...
            self.vertices.extend_from_slice(&[
                Vertex2D { position: center, color: c },

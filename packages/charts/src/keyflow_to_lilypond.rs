...
pub fn chart_to_lilypond(chart: &Chart) -> String {
    ...
    if let Some(ts) = chart.initial_time_signature.as_ref().or(chart.time_signature.as_ref()) {
...
fn rhythm_to_lilypond_duration(time_sig: &(u8, u8), _rhythm: Option<&keyflow::chord::ChordRhythm>) -> String {
...
fn format_section_mark(section: &keyflow::sections::Section, _section_idx: usize, _chart: &Chart) -> String {
...
pub fn render_chart_to_pdf(
    ...
    let complete_document = lilypond_builder::build_minimal_lilypond_document(&lilypond_content, output_dir);

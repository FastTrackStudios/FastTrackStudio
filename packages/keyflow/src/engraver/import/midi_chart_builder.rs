...
fn build_rhythm_elements(
    ...
    for cont in &continuing_chords {
        ...
        let already_covered = section_chords.iter().any(|sc| {
            (sc.start_ppq - section_start_tick).abs() < ticks_per_beat
        });
        if !already_covered {
...
fn build_measures(
    ...
    for measure_idx in 0..section_length_measures {
        ...
        for elem in elements {
            match elem {
                ...
                ChordOrRest::Rest {
                    ...
                    if start_beat_in_measure >= current_beat
        ...
        let content = if measure_elements.is_empty() {
            ...
            let is_chord_then_rest = matches!(measure_elements.first(), Some(MeasureElement::Chord { .. }))

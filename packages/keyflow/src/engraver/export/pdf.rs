...
impl PdfSerializer {
    ...
    pub fn serialize(&mut self, scene: &SceneNode) -> Result<Vec<u8>, PdfExportError> {
        ...
        if let (Some(font), Some(font_id)) = (&self.text_font, &self.text_font_id) {
            doc.resources.fonts.map.insert(font_id.clone(), font.clone());
        }
    ...
    pub fn serialize_from_png(
        ...
        let bytes = doc.with_pages(vec![page]).save(&save_options, &mut save_warnings);
    ...
    pub fn serialize_from_svg(
        ...
        for (i, svg_str) in svg_pages.iter().enumerate() {
            ...
            *options.fontdb_mut() = fontdb.clone();
            ...
            .map_err(|e| PdfExportError::SaveError(format!("SVG to PDF conversion failed: {e}")))?;
    ...
    fn render_paint_command(&self, ops: &mut Vec<Op>, cmd: &PaintCommand, transform: &Affine) {
        match cmd {
            ...
            PaintCommand::Rect {
                ...
                self.render_rect(ops, rect, transform, *fill, *stroke, *stroke_width, corner_radius.unwrap_or(0.0));
            }
            ...
            PaintCommand::Ellipse {
                ...
                let center_t = transform_point(transform, *center);
                self.render_ellipse(ops, center_t, *radius_x, *radius_y, *fill, *stroke, *stroke_width);
            }
        }
    }
    ...
    fn render_stroked_path(
        ...
        ops.push(Op::SetOutlineThickness { pt: Pt(width as f32) });

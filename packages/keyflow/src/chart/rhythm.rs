...
pub fn resolve_measure_rhythm(
    ...
    let has_any_triplet = beats_with_triplets.iter().any(|(is_triplet, _)| *is_triplet);
    ...
    for (beat_idx, (has_triplet, pushed_chord_idx)) in beats_with_triplets.iter().enumerate() {
        ...
        if beat_idx < measure.chords.len() {
            chord_to_segment.push(segment_idx - beats.last().map(BeatStructure::segment_count).unwrap_or(1));
        }
    }
...
pub fn measure_has_explicit_chord_rhythm(measure: &Measure) -> bool {
    ...
    measure.chords.iter().any(|chord| {
        matches!(chord.rhythm, ChordRhythm::Explicit(_))

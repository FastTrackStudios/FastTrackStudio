...
impl Note {
    ...
    pub fn c() -> Note { Note::new(NoteLiteral::C, None) }
    pub fn d() -> Note { Note::new(NoteLiteral::D, None) }
    pub fn e() -> Note { Note::new(NoteLiteral::E, None) }
    pub fn f() -> Note { Note::new(NoteLiteral::F, None) }
    pub fn g() -> Note { Note::new(NoteLiteral::G, None) }
    pub fn a() -> Note { Note::new(NoteLiteral::A, None) }
    pub fn b() -> Note { Note::new(NoteLiteral::B, None) }

    /// Parse a note from a string like "G", "C#", "Bb"
    pub fn from_string(note_str: &str) -> Option<Note> {
        ...
        let base_char = chars[0].to_ascii_uppercase();
        
        let literal = match base_char {
            'C' => NoteLiteral::C,
            'D' => NoteLiteral::D,
            'E' => NoteLiteral::E,
            'F' => NoteLiteral::F,
    ...
    pub fn transpose(&self, interval: Interval) -> Note {
        let semitone_offset = interval.st();
        let new_semitone = (self.to_semitone() + semitone_offset) % 12;
        
        // Find the note literal and modifier for the new semitone
        let note_literal = match new_semitone {
            0 => NoteLiteral::C,
            1 => NoteLiteral::C, // C#
            2 => NoteLiteral::D,
            3 => NoteLiteral::D, // D#
            ...
            10 => NoteLiteral::A, // A#
            11 => NoteLiteral::B,
            _ => NoteLiteral::C, // fallback
        };
        
        let modifier = match new_semitone {
            0 | 2 | 4 | 5 | 7 | 9 | 11 => None, // natural notes
            1 | 3 | 6 | 8 | 10 => Some(Modifier::Sharp), // sharp notes
            _ => None,
        };
        
        Note::new(note_literal, modifier)
    }
...
#[cfg(test)]
mod test {
    use crate::primitives::intervals::SemInterval;

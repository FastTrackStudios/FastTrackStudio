...
use crate::primitives::{intervals::Interval, note::Note};
...
impl SusExp {
    ...
    pub fn execute(&self, i: &mut Vec<Interval>) {
        // For suspended chords, we keep the actual interval values
        // (unlike extensions which map to higher octaves)
...
impl AltExp {
    pub fn execute(&self, i: &mut Vec<Interval>) {
        ...
        if !i.contains(&Interval::MinorSeventh) && !i.contains(&Interval::MajorSeventh) {
            i.push(Interval::MinorSeventh);
        }
        
        // Remove perfect fifth and add altered 5ths
        i.retain(|&interval| interval != Interval::PerfectFifth);
        i.push(Interval::DiminishedFifth);
        i.push(Interval::AugmentedFifth);

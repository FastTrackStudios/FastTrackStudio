...
//! - R key: Reset view
//!
//! Run with: cargo run -p keyflow --example canvas_sdf --features engraver-example
...
use wgpu::{
    BindGroup, BufferUsages, CommandEncoderDescriptor, DeviceDescriptor, Features, FragmentState,
    Instance, InstanceDescriptor, LoadOp, MultisampleState, Operations, PipelineLayoutDescriptor,
    PrimitiveState, PrimitiveTopology, RenderPassColorAttachment, RenderPassDescriptor,
    RenderPipeline, RenderPipelineDescriptor, RequestAdapterOptions, StoreOp,
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        for (text, x, y) in texts {
            ...
            println!(
                "Text '{}': text_pos=({}, {}), rect_pos=({}, {}), size=({:.1}x{:.1})",
        ...
        match event {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                ...
                if let Some(state) = &self.render_state {
                    ...
                    self.view.pan_x =
                        mouse_ndc_x - (mouse_ndc_x - self.view.pan_x) * (new_zoom / self.view.zoom);
                    self.view.pan_y =
                        mouse_ndc_y - (mouse_ndc_y - self.view.pan_y) * (new_zoom / self.view.zoom);
                }
            ...
            WindowEvent::RedrawRequested => {
                if let Some(state) = &mut self.render_state {
                    ...
                    let text_areas: Vec<TextArea> = state
                        .text_state
                        .elements
                        .iter()
                        .map(|elem| {

...
//! - R key: Reset view
//!
//! Run with: cargo run -p keyflow --example music_symbols --features engraver-example
...
use wgpu::{
    BindGroup, BindGroupLayout, BufferUsages, Color, CommandEncoderDescriptor, DeviceDescriptor,
    Features, FragmentState, Instance, InstanceDescriptor, LoadOp, MultisampleState, Operations,
...
impl App {
    fn new() -> Self {
        ...
        log::info!(
            ...
            page_style.size.width,
            page_style.size.height,
            page_style.size.width * MM_PER_INCH,
            page_style.size.height * MM_PER_INCH
        );
        log::info!(
            "Spatium: {:.3}mm, Staff height: {:.1}mm",
            page_style.spatium_mm,
            page_style.spatium_mm * 4.0
        );
...
fn create_rounded_frame_rect(
    ...
    let create_arc = |cx: f32, cy: f32, start_angle: f32, end_angle: f32| -> Vec<Vertex> {
        ...
        for i in 0..segments {
            ...
            arc_verts.extend_from_slice(&create_line(
                x0, y0, x1, y1, thickness, color, width, height,
            ));
        }
        arc_verts
    };
    ...
    vertices.extend_from_slice(&create_line(
        x + r,
        y,
        x + w - r,
        y,
...
impl DemoSong {
    fn new() -> Self {
        Self {
            sections: vec![("INTRO", 2), ("VS 1", 8), ("CH 1", 4), ("OUTRO", 4)],
        }
    }
...
fn compute_system_sky_line(
    staff_y: f32,
    staff_space: f32,
    sys_info: &keyflow::engraver::model::SystemInfo,
) -> f32 {
...
fn compute_rehearsal_positions(
    ...
    use keyflow::engraver::model::{LineBreakPolicy, compute_system_layout};
...
fn build_sdf_frames_from_computed(
    marks: &[ComputedRehearsalMark],
    window_width: f32,
    window_height: f32,
) -> Vec<SdfRectVertex> {
    let mut vertices = Vec::new();
...
fn create_debug_corners(
    ...
    // Top-left
    vertices.extend_from_slice(&create_rect(
        x,
        y,
        corner_size,
        corner_size,
    ...
    for (sys_idx, sys_info) in layout.systems.iter().enumerate() {
        ...
        if let Some(loaded_font) = font {
            ...
            if let Some(gid) = get_glyph_id(&font_ref, Glyph::GClef) {
                ...
                let clef_verts = tessellate_glyph(
                    &font_ref,
                    gid,
                    font_size,
                    clef_x,
            ...
            for m in 0..measures_in_system {
                ...
                for beat in 0..4 {
                    ...
                    let note_x = measure_start.max(note_area_start)
                        + beat_offset * (measure_end - measure_start.max(note_area_start)) * 0.8;
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let window_attrs = Window::default_attributes()
            .with_title(
                "Engraver - SMuFL Music Notation (Scroll: Pan, Opt+Scroll/Pinch: Zoom, R: Reset)",
            )
        ...
        log::info!(
            "FontSystem initialized with {} fonts",
            font_system.db().len()
        );
        ...
        let base_positions =
            compute_rehearsal_positions(&self.page_style, WINDOW_WIDTH as f32, self.rehearsal_mode);
        ...
        for pos in &base_positions {
            ...
            let (text_x, text_y, capsule_x, capsule_y, capsule_width, capsule_height, text_scale) =
                match self.rehearsal_mode {
                    RehearsalMarkMode::AboveStaff => {
            ...
            log::info!(
                "Rehearsal '{}' ({:?}): text=({:.1}, {:.1}, {:.1}x{:.1}) scale={:.2} -> capsule=({:.1}, {:.1}, {:.1}x{:.1})",
                pos.name,
                self.rehearsal_mode,
                text_x,
                text_y,
        ...
        let sdf_vertices = build_sdf_frames_from_computed(
            &computed_marks,
            WINDOW_WIDTH as f32,
            WINDOW_HEIGHT as f32,
        );
        ...
        log::info!(
            "SDF pipeline created with {} vertices for {} rehearsal frames",
            sdf_num_vertices,
            computed_marks.len()
        );

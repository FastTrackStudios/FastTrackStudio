...
//! - R key: Reset view
//!
//! Run with: cargo run -p keyflow --example canvas_shapes --features engraver-example

use std::sync::Arc;

use keyflow::engraver::renderer::canvas2d::{Canvas2D, Color, Rect, Vertex2D};
use wgpu::{
    BindGroup, BindGroupLayout, BufferUsages, CommandEncoderDescriptor, DeviceDescriptor, Features,
...
fn build_shapes(width: f32, height: f32) -> Vec<Vertex2D> {
    ...
    canvas.fill_rect(
        Rect::new(300.0, 100.0, 100.0, 150.0),
    ...
    canvas.stroke_rect(
        Rect::new(300.0, 280.0, 100.0, 100.0),
        3.0,

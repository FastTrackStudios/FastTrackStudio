...
use crate::types::{PdfPageData, RotationAngle, TextBounds, TextElement};
...
pub fn render_pdf_page_with_text(
    pdf_path: &str,
    page_index: usize,
    rotation: RotationAngle,
) -> Result<PdfPageData> {
    debug!(
        "Rendering PDF page {} from: {} (rotation: {:?})",
        page_index, pdf_path, rotation
    );

    let library_path = get_pdfium_library_path().context("Failed to locate PDFium library")?;
    ...
    let bitmap = page.render_with_config(&render_config).with_context(|| {
        format!(
            "Failed to render page {} with rotation {:?}",
            page_index, rotation
        )
    })?;
    debug!(
        "Page {} rendered successfully ({}x{})",
        page_index,
        bitmap.width(),
        bitmap.height()
    );

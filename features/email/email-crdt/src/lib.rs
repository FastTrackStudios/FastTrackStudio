//! Loro-backed `EmailRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `email-db`.
...
/// Newtype-wrapped repo, implements `EmailRepo`. Construct from a
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "email_items" LoroMap keyed by
...
impl EntityCrdt for EmailEntity {
    ...
    const ROOT: &'static str = "email_items";

    fn id(wire: &Email) -> Uuid {
        wire.id
    }
    ...
    fn encode_into(map: &LoroMap, e: &Email) -> Result<(), RepoError> {
    ...
    fn apply_update(m: &LoroMap, input: EmailUpdate) -> Result<(), RepoError> {

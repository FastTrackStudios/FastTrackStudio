//! `conference-proto` — wire contract for the `conference` feature.
//!
//! Rename the `Conference` placeholder below to whatever the canonical
...
#[architect(table_name = "conference_items", repo)]
...
pub trait ConferenceService {
    ...
    async fn ping(&self) -> Result<String, ConferenceServiceError>;
}

//! Native integration tests for the `conference` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use conference_crdt::{ConferenceRepoLoro, CrdtDoc};
use conference_proto::{ConferenceCreate, ConferenceRepo};

fn repo() -> ConferenceRepoLoro {
    ConferenceRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(ConferenceCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(ConferenceCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

//! Native integration tests for the `person` feature. Drives
...
use person_proto::{PersonCreate, PersonRepo};

fn repo() -> PersonRepoLoro {
    PersonRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(PersonCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(PersonCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

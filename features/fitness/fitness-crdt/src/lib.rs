//! Loro-backed `FitnessRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `fitness-db`.

use architect::{Page, RepoError, SortOrder};
use chrono::{DateTime, Utc};
use crdt::EntityCrdt;
use fitness_proto::{Fitness, FitnessCreate, FitnessList, FitnessRepo, FitnessUpdate};
...
pub struct FitnessEntity;

/// Newtype-wrapped repo, implements `FitnessRepo`. Construct from a
...
#[derive(Clone)]
pub struct FitnessRepoLoro {
    inner: LoroRepo<FitnessEntity>,
}

impl FitnessRepoLoro {
    pub fn new(doc: &CrdtDoc) -> Self {
        Self { inner: doc.repo() }
    }

    pub fn inner(&self) -> &LoroRepo<FitnessEntity> {
        &self.inner
    }
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "fitness_items" LoroMap keyed by
...
impl EntityCrdt for FitnessEntity {
    type Wire = Fitness;
    type Create = FitnessCreate;
    type Update = FitnessUpdate;
    type List = FitnessList;

    const ROOT: &'static str = "fitness_items";

    fn id(wire: &Fitness) -> Uuid {
        wire.id
    }

    fn from_create(input: FitnessCreate) -> Fitness {
        let now = Utc::now();
        Fitness {
            id: Uuid::new_v4(),
            name: input.name,
    ...
    fn encode_into(map: &LoroMap, e: &Fitness) -> Result<(), RepoError> {
    ...
    fn decode_from(m: &LoroMap) -> Result<Fitness, RepoError> {
        Ok(Fitness {
    ...
    fn apply_update(m: &LoroMap, input: FitnessUpdate) -> Result<(), RepoError> {
    ...
    fn sort_items(items: &mut [Fitness], field: &str, order: SortOrder) -> Result<(), RepoError> {
        match field {
    ...
    fn build_list(items: Vec<Fitness>, total: u32, page: Page) -> FitnessList {
        FitnessList { items, total, page }
    }
}

// ── FitnessRepo forwarders ────────────────────────────────────────────

impl FitnessRepo for FitnessRepoLoro {
    async fn get(&self, id: Uuid) -> Result<Fitness, RepoError> {
    ...
    ) -> Result<FitnessList, RepoError> {
        self.inner.list(page, sort, filter).await
    }

    async fn create(&self, input: FitnessCreate) -> Result<Fitness, RepoError> {
        self.inner.create(input).await
    }

    async fn update(&self, id: Uuid, input: FitnessUpdate) -> Result<Fitness, RepoError> {

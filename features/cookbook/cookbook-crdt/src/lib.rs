//! Loro-backed `CookbookRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `cookbook-db`.

use architect::{Page, RepoError, SortOrder};
use chrono::{DateTime, Utc};
use cookbook_proto::{Cookbook, CookbookCreate, CookbookList, CookbookRepo, CookbookUpdate};
use crdt::EntityCrdt;
...
/// Newtype-wrapped repo, implements `CookbookRepo`. Construct from a
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "cookbook_items" LoroMap keyed by
...
impl EntityCrdt for CookbookEntity {
    ...
    const ROOT: &'static str = "cookbook_items";

    fn id(wire: &Cookbook) -> Uuid {
        wire.id
    }
    ...
    fn encode_into(map: &LoroMap, e: &Cookbook) -> Result<(), RepoError> {
    ...
    fn apply_update(m: &LoroMap, input: CookbookUpdate) -> Result<(), RepoError> {

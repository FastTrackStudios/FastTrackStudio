//! Native integration tests for the `cookbook` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use cookbook_crdt::{CookbookRepoLoro, CrdtDoc};
use cookbook_proto::{CookbookCreate, CookbookRepo};

fn repo() -> CookbookRepoLoro {
    CookbookRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(CookbookCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(CookbookCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

//! Native integration tests for the `chat` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use chat_crdt::{ChatRepoLoro, CrdtDoc};
use chat_proto::{ChatCreate, ChatRepo};

fn repo() -> ChatRepoLoro {
    ChatRepoLoro::new(&CrdtDoc::ephemeral())
}

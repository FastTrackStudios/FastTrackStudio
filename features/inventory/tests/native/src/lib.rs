//! Native integration tests for the `inventory` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use inventory_crdt::{CrdtDoc, InventoryRepoLoro};
use inventory_proto::{InventoryCreate, InventoryRepo};

fn repo() -> InventoryRepoLoro {
    InventoryRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(InventoryCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(InventoryCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

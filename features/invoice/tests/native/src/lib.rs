//! Native integration tests for the `invoice` feature. Drives
...
use invoice_proto::{InvoiceCreate, InvoiceRepo};

fn repo() -> InvoiceRepoLoro {
    InvoiceRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(InvoiceCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(InvoiceCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

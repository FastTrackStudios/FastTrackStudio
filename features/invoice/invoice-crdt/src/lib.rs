//! Loro-backed `InvoiceRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `invoice-db`.

use architect::{Page, RepoError, SortOrder};
use chrono::{DateTime, Utc};
use crdt::EntityCrdt;
use invoice_proto::{Invoice, InvoiceCreate, InvoiceList, InvoiceRepo, InvoiceUpdate};
...
/// Newtype-wrapped repo, implements `InvoiceRepo`. Construct from a
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "invoice_items" LoroMap keyed by
...
impl EntityCrdt for InvoiceEntity {
    ...
    const ROOT: &'static str = "invoice_items";

    fn id(wire: &Invoice) -> Uuid {
        wire.id
    }
    ...
    fn encode_into(map: &LoroMap, e: &Invoice) -> Result<(), RepoError> {
    ...
    fn apply_update(m: &LoroMap, input: InvoiceUpdate) -> Result<(), RepoError> {

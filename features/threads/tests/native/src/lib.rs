//! Native integration tests for the `threads` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use threads_crdt::{CrdtDoc, ThreadsRepoLoro};
use threads_proto::{ThreadsCreate, ThreadsRepo};

fn repo() -> ThreadsRepoLoro {
    ThreadsRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(ThreadsCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(ThreadsCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

//! Native integration tests for the `timer` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use timer_crdt::{CrdtDoc, TimerRepoLoro};
use timer_proto::{TimerCreate, TimerRepo};

fn repo() -> TimerRepoLoro {
    TimerRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(TimerCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(TimerCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

//! Loro-backed `AgentRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `agent-db`.

use agent_proto::{Agent, AgentCreate, AgentList, AgentRepo, AgentUpdate};
...
/// Newtype-wrapped repo, implements `AgentRepo`. Construct from a
...
#[derive(Clone)]
pub struct AgentRepoLoro {
    inner: LoroRepo<AgentEntity>,
}
...
impl AgentRepoLoro {
    ...
    pub fn inner(&self) -> &LoroRepo<AgentEntity> {
        &self.inner
    }
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "agent_items" LoroMap keyed by
...
impl EntityCrdt for AgentEntity {
    type Wire = Agent;
    type Create = AgentCreate;
    type Update = AgentUpdate;
    type List = AgentList;

    const ROOT: &'static str = "agent_items";

    fn id(wire: &Agent) -> Uuid {
        wire.id
    }

    fn from_create(input: AgentCreate) -> Agent {
        let now = Utc::now();
        Agent {
            id: Uuid::new_v4(),
            name: input.name,
    ...
    fn encode_into(map: &LoroMap, e: &Agent) -> Result<(), RepoError> {
    ...
    fn decode_from(m: &LoroMap) -> Result<Agent, RepoError> {
        Ok(Agent {
    ...
    fn apply_update(m: &LoroMap, input: AgentUpdate) -> Result<(), RepoError> {
    ...
    fn sort_items(items: &mut [Agent], field: &str, order: SortOrder) -> Result<(), RepoError> {
        match field {
    ...
    fn build_list(items: Vec<Agent>, total: u32, page: Page) -> AgentList {
        AgentList { items, total, page }
    }
}

// ── AgentRepo forwarders ────────────────────────────────────────────

impl AgentRepo for AgentRepoLoro {
    async fn get(&self, id: Uuid) -> Result<Agent, RepoError> {
    ...
    ) -> Result<AgentList, RepoError> {
        self.inner.list(page, sort, filter).await
    }

    async fn create(&self, input: AgentCreate) -> Result<Agent, RepoError> {
        self.inner.create(input).await
    }

    async fn update(&self, id: Uuid, input: AgentUpdate) -> Result<Agent, RepoError> {

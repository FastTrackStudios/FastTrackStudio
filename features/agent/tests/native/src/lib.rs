//! Native integration tests for the `agent` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use agent_crdt::{AgentRepoLoro, CrdtDoc};
use agent_proto::{AgentCreate, AgentRepo};

fn repo() -> AgentRepoLoro {
    AgentRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(AgentCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(AgentCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

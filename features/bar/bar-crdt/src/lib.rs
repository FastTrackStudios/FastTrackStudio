//! Loro-backed `BarRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `bar-db`.

use architect::{Page, RepoError, SortOrder};
use bar_proto::{Bar, BarCreate, BarList, BarRepo, BarUpdate};
...
/// Newtype-wrapped repo, implements `BarRepo`. Construct from a
...
#[derive(Clone)]
pub struct BarRepoLoro {
    inner: LoroRepo<BarEntity>,
}
...
impl BarRepoLoro {
    ...
    pub fn inner(&self) -> &LoroRepo<BarEntity> {
        &self.inner
    }
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "bar_items" LoroMap keyed by
...
impl EntityCrdt for BarEntity {
    type Wire = Bar;
    type Create = BarCreate;
    type Update = BarUpdate;
    type List = BarList;

    const ROOT: &'static str = "bar_items";

    fn id(wire: &Bar) -> Uuid {
        wire.id
    }

    fn from_create(input: BarCreate) -> Bar {
        let now = Utc::now();
        Bar {
            id: Uuid::new_v4(),
            name: input.name,
    ...
    fn encode_into(map: &LoroMap, e: &Bar) -> Result<(), RepoError> {
    ...
    fn decode_from(m: &LoroMap) -> Result<Bar, RepoError> {
        Ok(Bar {
    ...
    fn apply_update(m: &LoroMap, input: BarUpdate) -> Result<(), RepoError> {
    ...
    fn sort_items(items: &mut [Bar], field: &str, order: SortOrder) -> Result<(), RepoError> {
        match field {
    ...
    fn build_list(items: Vec<Bar>, total: u32, page: Page) -> BarList {
        BarList { items, total, page }
    }
}

// ── BarRepo forwarders ────────────────────────────────────────────

impl BarRepo for BarRepoLoro {

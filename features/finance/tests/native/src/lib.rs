//! Native integration tests for the `finance` feature. Drives
//! the wire contract against the Loro-backed implementation through
//! an ephemeral `CrdtDoc`.

#![cfg(test)]

use finance_crdt::{CrdtDoc, FinanceRepoLoro};
use finance_proto::{FinanceCreate, FinanceRepo};

fn repo() -> FinanceRepoLoro {
    FinanceRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(FinanceCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(FinanceCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

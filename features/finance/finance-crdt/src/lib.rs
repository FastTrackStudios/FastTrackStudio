//! Loro-backed `FinanceRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `finance-db`.

use architect::{Page, RepoError, SortOrder};
use chrono::{DateTime, Utc};
use crdt::EntityCrdt;
use finance_proto::{Finance, FinanceCreate, FinanceList, FinanceRepo, FinanceUpdate};
...
/// Newtype-wrapped repo, implements `FinanceRepo`. Construct from a
...
#[derive(Clone)]
pub struct FinanceRepoLoro {
    inner: LoroRepo<FinanceEntity>,
}

impl FinanceRepoLoro {
    pub fn new(doc: &CrdtDoc) -> Self {
        Self { inner: doc.repo() }
    }

    pub fn inner(&self) -> &LoroRepo<FinanceEntity> {
        &self.inner
    }
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "finance_items" LoroMap keyed by
...
impl EntityCrdt for FinanceEntity {
    type Wire = Finance;
    type Create = FinanceCreate;
    type Update = FinanceUpdate;
    type List = FinanceList;

    const ROOT: &'static str = "finance_items";

    fn id(wire: &Finance) -> Uuid {
        wire.id
    }

    fn from_create(input: FinanceCreate) -> Finance {
        let now = Utc::now();
        Finance {
            id: Uuid::new_v4(),
            name: input.name,
    ...
    fn encode_into(map: &LoroMap, e: &Finance) -> Result<(), RepoError> {
    ...
    fn decode_from(m: &LoroMap) -> Result<Finance, RepoError> {
        Ok(Finance {
    ...
    fn apply_update(m: &LoroMap, input: FinanceUpdate) -> Result<(), RepoError> {
    ...
    fn sort_items(items: &mut [Finance], field: &str, order: SortOrder) -> Result<(), RepoError> {
        match field {
    ...
    fn build_list(items: Vec<Finance>, total: u32, page: Page) -> FinanceList {
        FinanceList { items, total, page }
    }
}

// ── FinanceRepo forwarders ────────────────────────────────────────────

impl FinanceRepo for FinanceRepoLoro {
    async fn get(&self, id: Uuid) -> Result<Finance, RepoError> {
    ...
    ) -> Result<FinanceList, RepoError> {
        self.inner.list(page, sort, filter).await
    }

    async fn create(&self, input: FinanceCreate) -> Result<Finance, RepoError> {
        self.inner.create(input).await
    }

    async fn update(&self, id: Uuid, input: FinanceUpdate) -> Result<Finance, RepoError> {

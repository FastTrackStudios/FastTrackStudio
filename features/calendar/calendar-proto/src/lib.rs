//! `calendar-proto` — wire contract for the `calendar` feature.
//!
//! Rename the `Calendar` placeholder below to whatever the canonical
...
#[architect(table_name = "calendar_items", repo)]
pub struct Calendar {
...
pub trait CalendarService {
    ...
    async fn ping(&self) -> Result<String, CalendarServiceError>;
}

//! Loro-backed `ProjectRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `project-db`.
...
/// Newtype-wrapped repo, implements `ProjectRepo`. Construct from a
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "project_items" LoroMap keyed by
...
impl EntityCrdt for ProjectEntity {
    ...
    const ROOT: &'static str = "project_items";

    fn id(wire: &Project) -> Uuid {
        wire.id
    }
    ...
    fn encode_into(map: &LoroMap, e: &Project) -> Result<(), RepoError> {
    ...
    fn apply_update(m: &LoroMap, input: ProjectUpdate) -> Result<(), RepoError> {

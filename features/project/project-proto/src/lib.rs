...
// ── WorkspaceSync ──────────────────────────────────────────────────────
//
// Loro-CRDT-over-vox. The server side bridges
// `LoroDoc::subscribe_local_update` into a `Tx<Vec<u8>>` channel so
// clients can hold their own `LoroDoc` and merge updates as they
// arrive. `apply_update` is the reverse direction — clients push
// their committed-local bytes back up. The two together turn vox
// from a synchronous-RPC pipe into the transport for a real
// multi-peer CRDT mesh.
//
// The wire format is opaque Loro bytes; nothing here cares whether
// they're `ExportMode::Snapshot`, `ExportMode::Updates`, or anything
// else. The server is free to send a fresh snapshot whenever a slow
// subscriber lags out of the broadcast window.

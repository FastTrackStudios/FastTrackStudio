//! Native integration tests for the `project` feature. Drives
...
use project_proto::{ProjectCreate, ProjectRepo};

fn repo() -> ProjectRepoLoro {
    ProjectRepoLoro::new(&CrdtDoc::ephemeral())
}
...
async fn two_replicas_converge_after_sync() {
    ...
    let a = repo();
    let b = repo();
    a.create(ProjectCreate {
        name: "from-a".into(),
    })
    .await
    .unwrap();
    b.create(ProjectCreate {
        name: "from-b".into(),
    })
    .await
    .unwrap();

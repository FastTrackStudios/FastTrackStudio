//! In-memory `FooRepo` implementation.
...
use chrono::Utc;
use foo_proto::{
    Foo, FooCreate, FooList, FooRepo, FooUpdate,
};
use tokio::sync::RwLock;
use uuid::Uuid;

#[derive(Clone, Default)]
pub struct FooRepoMemory {
    inner: Arc<RwLock<Vec<Foo>>>,
}
...
impl FooRepo for FooRepoMemory {
    ...
    async fn list(
        ...
        let mut items: Vec<Foo> = self.inner.read().await.iter().cloned().collect();

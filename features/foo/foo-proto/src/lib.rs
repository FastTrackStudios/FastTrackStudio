//! `foo-proto` — wire contract for the `foo` feature.
//!
//! Rename the `Foo` placeholder below to whatever the canonical
...
#[architect(table_name = "foo_items", repo)]
pub struct Foo {
...
pub trait FooService {
    ...
    async fn ping(&self) -> Result<String, FooServiceError>;
}

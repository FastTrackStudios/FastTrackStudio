//! Loro-backed `AssetRepo`. Source of truth lives in a LoroDoc;
//! persistence is the concern of `asset-db`.
...
/// Newtype-wrapped repo, implements `AssetRepo`. Construct from a
...
// ── EntityCrdt impl ───────────────────────────────────────────────────
//
// Container layout: a top-level "asset_items" LoroMap keyed by
...
impl EntityCrdt for AssetEntity {
    ...
    const ROOT: &'static str = "asset_items";

    fn id(wire: &Asset) -> Uuid {
        wire.id
    }
    ...
    fn encode_into(map: &LoroMap, e: &Asset) -> Result<(), RepoError> {
    ...
    fn apply_update(m: &LoroMap, input: AssetUpdate) -> Result<(), RepoError> {

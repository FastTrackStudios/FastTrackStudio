...
//!
//! Contains the `DragMode` enum (pan / module-drag / node-drag), the
//! `Selection` model, grid-snap logic, and viewport fit calculations.
...
pub(crate) const GRID_SNAP: f64 = 20.0;

pub(crate) fn snap_to_grid(val: f64) -> f64 {
    (val / GRID_SNAP).round() * GRID_SNAP
}

// ── Canvas Bounds ────────────────────────────────────────────────────

pub(crate) fn calculate_canvas_bounds(graph: &NodeGraph) -> (f64, f64) {

...
use crate::types::time_tempo::{TempoTimePoint, TempoTimeEnvelope};
...
mod tests {
    ...
    fn calculate_beat_position_constant_tempo(time_seconds: f64, tempo_bpm: f64, time_signature: (i32, i32)) -> (i32, i32, f64) {
        ...
        let beat_fraction = (total_beats % beats_per_measure) - (total_beats % beats_per_measure).floor();
    ...
    fn calculate_beat_position_with_changes(
        ...
        println!("  Starting at measure {}, beat {}, fraction {:.3}", current_measure, current_beat, current_beat_fraction);
        ...
        for &(point_time, point_tempo, point_time_sig) in change_points.iter() {
            if point_time <= time_seconds {
                ...
                while current_beat_fraction >= 1.0 {
                    ...
                    println!("      After overflow: measure {}, beat {}, fraction {:.15}", current_measure, current_beat, current_beat_fraction);
                }
                
                println!("    After segment: measure {}, beat {}, fraction {:.3}", current_measure, current_beat, current_beat_fraction);
                ...
                println!("    Final result: measure {}, beat {}, fraction {:.3}", current_measure, current_beat, current_beat_fraction);
                found_target = true;
                break;
            }
        }
    ...
    fn calculate_beat_position_with_tempo_changes(
        ...
        let change_points: Vec<(f64, f64, (i32, i32))> = tempo_points.iter()
    ...
    fn test_constant_tempo_calculation() {
        ...
        for (time, expected) in test_cases {
            let (measure, beat, fraction) = calculate_beat_position_constant_tempo(time, tempo, time_sig);
            println!("Time {:.1}s: Measure {}, Beat {}, Fraction {:.3} (expected: {}.{}.{:02})", 
                     time, measure, beat, fraction, expected.0, expected.1, (expected.2 * 100.0) as i32);
    ...
    fn test_beat_calculation_math() {
        ...
        let (measure, beat, fraction) = calculate_beat_position_constant_tempo(0.6, tempo, time_sig);
        println!("0.6s: Measure {}, Beat {}, Fraction {:.3}", measure, beat, fraction);
        assert_eq!(measure, 1);
        assert_eq!(beat, 2);
        ...
        let (measure, beat, fraction) = calculate_beat_position_constant_tempo(1.2, tempo, time_sig);
        println!("1.2s: Measure {}, Beat {}, Fraction {:.3}", measure, beat, fraction);
        assert_eq!(measure, 1);
        assert_eq!(beat, 3);
    }
    ...
    fn test_tempo_change_calculation() {
        ...
        for (time, expected) in test_cases {
            ...
            let (measure, beat, fraction) = calculate_beat_position_with_tempo_changes(time, &tempo_points, time_sig);
            println!("Result: Measure {}, Beat {}, Fraction {:.3} (expected: {}.{}.{:02})", 
                     measure, beat, fraction, expected.0, expected.1, (expected.2 * 100.0) as i32);
    ...
    fn test_first_two_measures() {
        ...
        for (time, expected) in test_cases {
            println!("\nTesting time {:.1}s:", time);
            let (measure, beat, fraction) = calculate_beat_position_with_changes(time, &change_points, default_time_sig);
            println!("Result: Measure {}, Beat {}, Fraction {:.3} (expected: {}.{}.{:02})", 
                     measure, beat, fraction, expected.0, expected.1, (expected.2 * 100.0) as i32);
    ...
    fn test_different_time_signatures_effective_tempo() {
        ...
        for ((num, denom), tempo, expected_ratio) in test_cases {
            ...
            println!("Time signature {}/{}: {} BPM * {:.1} ratio = {:.1} effective BPM", 
                     num, denom, tempo, tempo_ratio, effective_tempo);
            
            assert!((tempo_ratio - expected_ratio).abs() < 0.01, 
    ...
    fn test_comprehensive_beat_positions() {
        ...
        let rpp_project = crate::parse_rpp_file(&rpp_content)
            .expect("Failed to parse RPP content");
        ...
        let envelope = project.tempo_envelope
        ...
        for marker in &project.markers_regions.all {
            if marker.is_marker() && !marker.name.is_empty() {
                ...
                println!("Testing M{}: time={:.3}s, expected={}, actual={}", 
        ...
        for (time, expected) in test_cases {
            ...
            println!("Time {:.1}s: expected {}, actual {}", time, expected, actual);
        }
        ...
        for region in &project.markers_regions.regions {
            if !region.name.is_empty() {
                ...
                let actual_end = time_to_beat_position_with_envelope(region.end_position.unwrap(), &envelope);
                println!("Testing R{}: start={:.3}s->{}, end={:.3}s->{}", 
                         region.id, region.position, actual_start, region.end_position.unwrap(), actual_end);
            }
        }
    }
    ...
    fn test_extend_to_measure_5_1() {
        ...
        for (time, expected) in test_cases {
            let (measure, beat, beat_fraction) = calculate_beat_position_with_changes(
                time, &change_points, (4, 4)
            );
            let subbeat = (beat_fraction * 100.0).round() as i32;

...
impl MarkerRegion {
    ...
    pub fn from_marker_line(line: &str) -> Result<Self, String> {
        let (_remaining, tokens) = parse_token_line(line)
            .map_err(|e| format!("Failed to parse marker line: {:?}", e))?;
        
        // Note: remaining input is expected for marker lines as they may have trailing whitespace
        
        if tokens.len() < 5 {
            return Err(format!("Marker line has insufficient tokens: expected at least 5, got {}: {:?}", tokens.len(), tokens));
        }
        ...
        let color = if tokens.len() > 4 { tokens[4].as_number().unwrap_or(0.0) as i32 } else { 0 };
        let flags = if tokens.len() > 5 { tokens[5].as_number().unwrap_or(0.0) as i32 } else { 0 };
        let locked = if tokens.len() > 6 { tokens[6].as_number().unwrap_or(0.0) as i32 } else { 0 };
        // Skip tokens[7] which appears to be a single character (possibly a flag)
        let guid = if tokens.len() > 8 { tokens[8].as_string().unwrap_or("").to_string() } else { "".to_string() };
        let additional = if tokens.len() > 9 { tokens[9].as_number().unwrap_or(0.0) as i32 } else { 0 };
        
        // Check if this is a region (has end position)
        let end_position = if tokens.len() > 10 {
            Some(tokens[10].as_number().ok_or("Invalid region end position")?)
        } else {
            None
        };
    ...
    pub fn musical_position(&self, bpm: f64, time_signature_numerator: i32, _time_signature_denominator: i32) -> (i32, i32, f64) {
    ...
    pub fn musical_position_string(&self, bpm: f64, time_signature_numerator: i32, time_signature_denominator: i32) -> String {
        let (measure, beat, fraction) = self.musical_position(bpm, time_signature_numerator, time_signature_denominator);
    ...
    pub fn calculate_beat_position(&mut self, tempo_envelope: &crate::types::time_tempo::TempoTimeEnvelope) {
        use crate::types::time_pos_utils::time_to_beat_position_with_envelope;
        self.beat_position = Some(time_to_beat_position_with_envelope(self.position, tempo_envelope));
    }
    ...
    pub fn calculate_beat_position_for_time(time: f64, tempo_envelope: &crate::types::time_tempo::TempoTimeEnvelope) -> String {
        use crate::types::time_pos_utils::time_to_beat_position_with_envelope;
        time_to_beat_position_with_envelope(time, tempo_envelope)
    }
    
    /// Get the beat position, calculating it if not already set
    pub fn get_beat_position(&self, tempo_envelope: &crate::types::time_tempo::TempoTimeEnvelope) -> String {
...
impl MarkerRegion {
    ...
    pub fn display_with_musical_position(&self, bpm: f64, time_signature_numerator: i32, _time_signature_denominator: i32) -> String {
        let marker_type = if self.is_region() { "Region" } else { "Marker" };
        let musical_pos = self.musical_position_string(bpm, time_signature_numerator, _time_signature_denominator);
        let mut output = format!("{} #{}: \"{}\"\n", marker_type, self.id, self.name);
        output.push_str(&format!("  Position: {:.3}s ({})\n", self.position, musical_pos));
        ...
        if let Some(end) = self.end_position {
            let end_musical_pos = MarkerRegion {
                ...
                end_position: None,
                beat_position: None,
            }.musical_position_string(bpm, time_signature_numerator, _time_signature_denominator);
...
impl MarkerRegionCollection {
    ...
    pub fn add(&mut self, marker_region: MarkerRegion) {
        ...
        self.all.retain(|m| !(m.id == marker_region.id && m.position == marker_region.position));
        self.markers.retain(|m| !(m.id == marker_region.id && m.position == marker_region.position));
        self.regions.retain(|r| !(r.id == marker_region.id && r.position == marker_region.position));
    ...
    pub fn process_regions(&mut self) {
        ...
        for (_id, mut markers) in markers_by_id {
            if markers.len() >= 2 {
                // Sort by position
                markers.sort_by(|a, b| a.position.partial_cmp(&b.position).unwrap_or(std::cmp::Ordering::Equal));
                ...
                for i in 0..markers.len() - 1 {
                    ...
                    if !start.name.is_empty() && end.name.is_empty() && start.position < end.position {
    ...
    pub fn get(&self, id: i32, position: f64) -> Option<&MarkerRegion> {
        self.all.iter().find(|m| m.id == id && m.position == position)
    }
    ...
    pub fn all_sorted(&self) -> Vec<&MarkerRegion> {
        ...
        all.sort_by(|a, b| a.position.partial_cmp(&b.position).unwrap_or(std::cmp::Ordering::Equal));
        all
    }
    ...
    pub fn regions_sorted(&self) -> Vec<&MarkerRegion> {
        ...
        regions.sort_by(|a, b| a.position.partial_cmp(&b.position).unwrap_or(std::cmp::Ordering::Equal));
        regions
    }
    
    /// Calculate beat positions for all markers and regions using tempo envelope data
    pub fn calculate_beat_positions(&mut self, tempo_envelope: &crate::types::time_tempo::TempoTimeEnvelope) {
    ...
    pub fn display_with_musical_positions(&self, bpm: f64, time_signature_numerator: i32, time_signature_denominator: i32) -> String {
        ...
        if !self.all.is_empty() {
            ...
            for marker_region in self.all_sorted() {
                output.push_str(&marker_region.display_with_musical_position(bpm, time_signature_numerator, time_signature_denominator));
            }
        }
    ...
    pub fn display_with_beat_positions(&self, tempo_envelope: &crate::types::time_tempo::TempoTimeEnvelope) -> String {
        let mut output = String::new();
        output.push_str(&format!("Markers and Regions ({} total)\n", self.all.len()));
        output.push_str(&format!("  Markers: {}\n", self.markers.len()));
        output.push_str(&format!("  Regions: {}\n", self.regions.len()));
        ...
        if !self.all.is_empty() {
            output.push('\n');
            for marker_region in self.all_sorted() {
                let marker_type = if marker_region.is_region() { "Region" } else { "Marker" };
                let beat_pos = marker_region.get_beat_position(tempo_envelope);
                output.push_str(&format!("{} #{}: \"{}\"\n", marker_type, marker_region.id, marker_region.name));
                output.push_str(&format!("  Position: {:.3}s (Beat Position: {})\n", marker_region.position, beat_pos));
                
                if let Some(end) = marker_region.end_position {
                    let end_beat_pos = MarkerRegion::calculate_beat_position_for_time(end, tempo_envelope);
                    output.push_str(&format!("  End: {:.3}s (Beat Position: {})\n", end, end_beat_pos));
...
mod tests {
    ...
    fn test_parse_marker() {
        let line = r#"MARKER 1 0.98405631332507 =START 0 0 1 B {976796DE-F915-A9CD-2372-4ED6EF87EE3F} 0"#;
    ...
    fn test_collection_operations() {
        ...
        let marker = MarkerRegion::from_marker_line(r#"MARKER 1 0.0 "Test Marker" 0 0 1 B {12345678-1234-1234-1234-123456789012} 0"#).unwrap();
        let region = MarkerRegion::from_marker_line(r#"MARKER 2 1.0 "Test Region" 0 0 1 B {87654321-4321-4321-4321-210987654321} 0 2.0"#).unwrap();
    ...
    fn test_musical_position() {
        let marker = MarkerRegion::from_marker_line(r#"MARKER 1 2.0 "Test Marker" 0 0 1 B {12345678-1234-1234-1234-123456789012} 0"#).unwrap();
        ...
        println!("Debug: 2 seconds at 120 BPM = {} beats, measure = {}, beat = {}, fraction = {}", 
                 2.0 * 120.0 / 60.0, measure, beat, fraction);
        ...
        println!("Debug: 2 seconds at 60 BPM = {} beats, measure = {}, beat = {}, fraction = {}", 
                 2.0 * 60.0 / 60.0, measure, beat, fraction);

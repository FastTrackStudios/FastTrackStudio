...
impl TempoTimePoint {
    ...
    pub fn from_pt_line(line: &str) -> Result<Self, String> {
        let (_remaining, tokens) = parse_token_line(line)
            .map_err(|e| format!("Failed to parse PT line: {:?}", e))?;
        
        if tokens.len() < 4 {
            return Err(format!("PT line has insufficient tokens: expected at least 4, got {}: {:?}", tokens.len(), tokens));
        }
...
impl TempoTimeEnvelope {
    ...
    pub fn beats_at_time(&self, time: f64) -> f64 {
        ...
        for (i, point) in self.points.iter().enumerate() {
            println!("  Point {}: {:.3}s, {:.1} BPM", i, point.position, point.tempo);
            ...
            if point.position <= time {
                ...
                println!("    Segment {:.3}s to {:.3}s: {:.3}s at {:.1} BPM = {:.3} beats", 
                ...
                println!("    Final segment {:.3}s to {:.3}s: {:.3}s at {:.1} BPM = {:.3} beats", 
...
mod tests {
    ...
    fn test_region_test_tempo_changes() {
        ...
        println!("  Measure: {}, Beat: {}, Fraction: {:.3}", measure, beat, fraction);

...
use super::block::{RppBlock, RppBlockContent, BlockType};
...
pub fn parse_project_header(input: &str) -> IResult<&str, (f64, String, i64)> {
    map(
        ...
        |(_, _, tokens)| {
            ...
            let version_string = tokens.get(1)
                .and_then(|t| t.as_string())
                .unwrap_or("")
                .to_string();
            
            let timestamp = tokens.get(2)
                .and_then(|t| t.as_number())
...
fn parse_project_content(input: &str) -> IResult<&str, Vec<RppBlock>> {
    ...
    loop {
        ...
        if let Ok((input_after_end, _)) = nom::bytes::complete::tag::<&str, &str, nom::error::Error<&str>>(">")(input_after_ws) {
        ...
        if let Ok((input_after_content, content_tokens)) = parse_token_line(input_after_ws) {
            project_block.children.push(RppBlockContent::Content(content_tokens));
            
            // Skip the newline after the content line
            let (input_after_newline, _) = nom::character::complete::multispace0(input_after_content)?;

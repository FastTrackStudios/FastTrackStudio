    ...
    T: TransportActions + Send + Sync,
{
    ...
    match transport.play() {
        Ok(message) => Ok(Json(ActionResponse { success: true, message })),
    ...
    T: TransportActions + Send + Sync,
{
    ...
    let record_mode = match payload.mode.to_lowercase().as_str() {
        ...
        _ => return Err((StatusCode::BAD_REQUEST, "Invalid record mode. Valid options: normal, time_selection, item".to_string())),
    };
    ...
    T: TransportActions + Send + Sync,
{
    ...
    let is_playing = transport.is_playing().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let is_recording = transport.is_recording().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let tempo = transport.get_tempo().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let position = transport.get_position().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let time_sig = transport.get_time_signature().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    let record_mode = transport.get_record_mode().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
{
    let transport = transport.lock().await;
    let is_ready = transport.is_ready().map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(is_ready))
}

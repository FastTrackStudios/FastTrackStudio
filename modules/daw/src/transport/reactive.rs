...
impl DefaultTransportReactiveService {
    ...
    fn emit_transport(&self, project_id: String, transport: Transport) {
        ...
        *emitting = true;
        drop(emitting);
        self.streams.transport_changed.borrow_mut().next((project_id, transport));
    ...
    fn emit_play_state(&self, project_id: String, play_state: PlayState) {
        ...
        *emitting = true;
        drop(emitting);
        self.streams.play_state_changed.borrow_mut().next((project_id, play_state));
        *self.emitting.lock().unwrap() = false;
    }
    ...
    fn emit_tempo(&self, project_id: String, tempo: Tempo) {
        ...
        *emitting = true;
        drop(emitting);
        self.streams.tempo_changed.borrow_mut().next((project_id, tempo));
        *self.emitting.lock().unwrap() = false;
    }
    ...
    fn emit_position(&self, project_id: String, position: f64) {
        ...
        *emitting = true;
        drop(emitting);
        self.streams.position_changed.borrow_mut().next((project_id, position));
        *self.emitting.lock().unwrap() = false;
    }
}

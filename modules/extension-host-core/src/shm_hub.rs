...
use crate::tracing_aggregator::TracingAggregator;
...
impl ShmHub {
    ...
    pub fn create(shm_path: &Path) -> Result<Self> {
        ...
        // Spawn driver task on tokio runtime
    ...
    pub async fn register_peer(&self, peer_id: u8, extension_name: Option<String>) -> Result<ConnectionHandle> {
        ...
        // Create host service dispatcher with access to the service registry
        let host_service_dispatcher = crate::host_service::create_host_dispatcher(
        ...
        // Try tracing first, then host service
        let combined_dispatcher = RoutedDispatcher::new(tracing_dispatcher, host_service_dispatcher);

        // Register peer with driver and get connection handle
        use roam_shm::peer::PeerId;
        let roam_peer_id = PeerId::from_index(peer_id)

...
use crate::ui::signals::{RIG_AVAILABLE_PRESETS, RIG_AVAILABLE_PROFILES, RIG_CURRENT_PRESET, RIG_CURRENT_PRESET_SNAPSHOT_ID, RIG_PROFILE};
use nucleo_matcher::{
...
fn TagsDisplay(props: TagsDisplayProps) -> Element {
    let tags: Vec<&Tag> = props
        .tag_ids
        .iter()
        .filter_map(|id| props.registry.get(crate::id::TagId::from_uuid(*id)))
        .take(3)
        .collect();

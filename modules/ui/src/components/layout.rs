...
use crate::components::song::{FadedSongTitle, SongTitle};
...
use fts::setlist::{SETLIST_STRUCTURE, SONG_TRANSPORT, Section, Setlist, Song};
...
pub fn TopBar(
    ...
    #[props(default = false)] is_server_mode: bool,
    #[props(default)] on_toggle_mode: Option<Callback<()>>,
    edit_mode: Signal<bool>,
    on_toggle_edit: Callback<()>,
    #[props(default)] edit_view_mode: Option<Signal<Option<EditViewMode>>>,
    #[props(default)] on_edit_view_change: Option<Callback<EditViewMode>>,
    #[props(default)] current_route_path: Option<String>,
) -> Element {
    rsx! {
        div {
            ...
            .map(|setlist| {
                setlist
                    ...
                    .map(|(idx, song)| {
                        ...
                        let song_is_playing = transport.map(|t| t.is_playing()).unwrap_or(false);
...
pub fn MainContent(
    ...
    on_back: Callback<()>,
    on_forward: Callback<()>,
    #[props(default)] on_seek_to_time: Option<Callback<(usize, f64)>>,
    #[props(default)] on_seek_to_musical_position: Option<
        Callback<(usize, daw::primitives::MusicalPosition)>,
    >,
) -> Element {
    ...
    let song_info = use_memo(move || {
        ...
        let song_total = setlist_structure
            .as_ref()
            .map(|setlist| setlist.songs.len());
        ...
        current_song_data()
            ...
            .map(|(song, current_pos)| {
                ...
                let format_section_name = |name: &str, is_last: bool| -> String {
                    ...
                    if name.eq_ignore_ascii_case("Count In")
                        || name.eq_ignore_ascii_case("Count-In")
                    {
                        return String::new();
                    }
                ...
                for section in song.sections.iter() {
                        ...
                        (section.start_seconds(), section.end_seconds())
                    {
                        let section_start_percent = if total_duration > 0.0 {
                            ((section_start - total_start) / total_duration * 100.0)
                                .clamp(0.0, 100.0)
                        } else {
                            0.0
                        };
                        ...
                        if has_ending_section && section_end_percent >= ending_start_percent {
                            ...
                            section_end_percent =
                                (ending_start_percent - 0.01).max(section_start_percent);
                        }
    ...
    let tempo_markers = use_memo(move || {
        current_song_data()
            ...
            .map(|(song, _)| {
                ...
                let total_end = if ending_pos > song.effective_end() {
                    ending_pos
                } else {
                    song.effective_end()
                };
                ...
                let first_tempo = song
                    .starting_tempo
                    .or_else(|| song.transport().map(|t| t.tempo.bpm));

                let first_time_sig = song
                ...
                for point in sorted_changes.iter() {
                    let current_tempo = if point.tempo > 0.0 {
                        Some(point.tempo)
                    } else {
                        None
                    };
                    ...
                    if tempo_changed || time_sig_changed {
                        ...
                        let marker_percent = if song_duration > 0.0 {
                            ((marker_position_absolute - total_start) / song_duration * 100.0)
                                .clamp(0.0, 100.0)
                        } else {
                            0.0
                        };
                            ...
                            if let Some((n, d)) = current_time_sig {
                                if let Some(prev_sig) = prev_time_sig_for_label {
                                    if prev_sig != (n, d) {
                                        ...
                                        let section_index =
                                            song.current_section_index(song_relative_pos);
                ...
                for (i, change) in tempo_changes.iter().enumerate() {
                    ...
                    let is_far_enough = if i == 0 {
                        ...
                        let measure_diff = (change.position.musical.measure
                            - prev_change.position.musical.measure)
                            .abs();

                        // Calculate time difference (seconds)
                        let time_diff =
                            (change.position_seconds - prev_change.position_seconds).abs();
                    ...
                    let is_short_large_change = if bpm_diff >= LARGE_BPM_CHANGE_THRESHOLD {
                        ...
                        if i + 1 < tempo_changes.len() {
                            ...
                            let time_diff_to_next =
                                (next_change.position_seconds - change.position_seconds).abs();
                    ...
                    let is_tempo_spike = if bpm_diff >= LARGE_BPM_CHANGE_THRESHOLD {
                        ...
                        for j in (0..i).rev() {
                            ...
                            let measure_diff =
                                (current_measure - other_change.position.musical.measure).abs();
                            if measure_diff <= 4 {
                        ...
                        for j in (i + 1)..tempo_changes.len() {
                            let other_change = &tempo_changes[j];
                            let measure_diff =
                                (other_change.position.musical.measure - current_measure).abs();
                            if measure_diff <= 4 {
                                nearby_tempos.push(other_change.tempo);
                                if nearby_tempos.len() >= 5 {
                                    break;
                        ...
                        if nearby_tempos.len() >= 2 {
                            ...
                            let avg_tempo =
                                nearby_tempos.iter().sum::<f64>() / nearby_tempos.len() as f64;
                ...
                final_filtered.sort_by(|&a, &b| {
                    tempo_changes[a]
                        .position_seconds
                        .partial_cmp(&tempo_changes[b].position_seconds)
                        .unwrap_or(std::cmp::Ordering::Equal)
                });
    ...
    let section_tempo_markers_and_measures = use_memo(move || {
        ...
        let section_tempo_markers: Vec<TempoMarker> = all_tempo_markers
            .iter()
            .filter(|marker| {
                marker.position_percent >= section_start_percent
                    && marker.position_percent < section_end_percent
            })
            .map(|marker| {
        ...
        if !song.measure_positions.is_empty() {
            ...
            let section_measures: Vec<_> = song
                .measure_positions
                .iter()
                .filter(|pos| {
            ...
            if !section_measures.is_empty() {
                ...
                for measure_pos in section_measures {
                    ...
                    let measure_progress_in_section = if section_duration > 0.0 {
                        ((measure_time - section_start_time) / section_duration * 100.0)
                            .clamp(0.0, 100.0)
                    } else {
                        0.0
                    };
            ...
            if let Some(chart_section) = chart.sections.iter().find(|cs| {
                // Match by section name or type (SectionType implements PartialEq)
                cs.section.name == section.name || cs.section.section_type == section.section_type
            }) {
                ...
                for (measure_idx, measure) in chart_section.measures.iter().enumerate() {
                    ...
                    let measure_time_sig =
                        if measure.time_signature.0 > 0 && measure.time_signature.1 > 0 {
                            (
                ...
                for (measure_idx, (measure_time_sig, qn_length)) in
                    measure_quarter_note_lengths.iter().enumerate()
                {
                    let measure_number_absolute = start_measure + measure_idx as i32;
                    ...
                    info!(
                        "  Measure {}: percent={:.2}%, time_sig={}/{}, quarter_notes={:.2}",
                        measure_number_absolute + 1,
                        measure_progress_in_section,
                        measure_time_sig.0,
                        measure_time_sig.1,
                        qn_length
                    );
                ...
                for (idx, (measure_time_sig, qn_length)) in
                    measure_quarter_note_lengths.iter().enumerate()
                {
    ...
    let section_tempo_markers = use_memo(move || {
        section_tempo_markers_and_measures()
            .as_ref()

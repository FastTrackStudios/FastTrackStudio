...
impl TrackVecHelpers for Vec<Track> {
    ...
    fn set_group(track: &mut Track, group: &str) {
        track.set_metadata("fts.group", group);
    }
    
    fn get_group(track: &Track) -> Option<String> {
        track.get_metadata("fts.group").cloned()
    }
}
...
pub fn tracks_eq(a: &[Track], b: &[Track]) -> bool {
    ...
    a.iter().zip(b.iter()).all(|(a, b)| {
        ...
        a.name == b.name
            && a.get_metadata("fts.track_type") == b.get_metadata("fts.track_type")
            && a.get_metadata("fts.parent") == b.get_metadata("fts.parent")
            && a.is_folder == b.is_folder
            && a.folder_depth_change == b.folder_depth_change
            && a.track_depth == b.track_depth
    })
}
...
pub trait TrackExt {
    ...
    fn get_modes(&self) -> Vec<crate::smart_template::shared::GroupMode>;
    
    /// Set group modes
    fn set_modes(&mut self, modes: &[crate::smart_template::shared::GroupMode]);
}

// We own TrackExt, so we can implement it directly for daw::tracks::Track
impl TrackExt for Track {
    fn track_type(&self) -> Option<&str> {
        self.get_metadata("fts.track_type").map(|s| s.as_str())
    }
    
    fn set_track_type(&mut self, track_type: &str) {
        self.set_metadata("fts.track_type", track_type);
    }
    
    fn parent_name(&self) -> Option<&str> {
        self.get_metadata("fts.parent").map(|s| s.as_str())
    }
    
    fn set_parent_name(&mut self, parent: &str) {
        self.set_metadata("fts.parent", parent);
    }
    ...
    fn get_modes(&self) -> Vec<crate::smart_template::shared::GroupMode> {
        ...
        if let Some(modes_str) = self.get_metadata("fts.modes") {
            modes_str.split(',')
                .filter_map(|s| {
                    let s = s.trim();
                    if s.is_empty() {
                        ...
                        // Parse as GroupMode
                        crate::smart_template::shared::GroupMode::from_str(s)
                    }
                })
                .collect()
        } else {
    ...
    fn set_modes(&mut self, modes: &[crate::smart_template::shared::GroupMode]) {
        let modes_str = modes.iter()
            ...
            .collect::<Vec<_>>()
            .join(",");
        self.set_metadata("fts.modes", &modes_str);
    }
}

...
use crate::smart_template::features::naming::component_order::{ComponentOrder, ComponentOrderType};
        ...
        for group in groups {
            ...
            let pattern_matches: usize = group.patterns.iter()
                .map(|pattern| {
                    ...
                    // Check if name starts with pattern (with or without space/number/dot)
                    if name_lower.starts_with(&pattern_lower) {
                        // Check if followed by space, number, dot, or end of string
                    ...
                    // Check if name contains pattern with word boundaries (space or dot)

...
use crate::smart_template::naming::components::{
    ComponentFoldParser,
    GroupFolder,
    DefaultGroupFolder,
};
...
impl ItemPropertiesParser {
    ...
    pub fn parse(&self, name: &str) -> ItemProperties {
        ...
        let matched_group = self.group_folder.fold_groups(name, &self.groups);
        ...
        let props = self.component_parser.parse(name, matched_group);

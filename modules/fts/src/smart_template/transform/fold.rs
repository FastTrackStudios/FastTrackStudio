...
use crate::smart_template::helpers::track_helpers::TrackExt;
...
pub struct FoldContext<'a> {
    ...
    pub tracks_by_name: HashMap<String, &'a Track>,
...
impl<'a> FoldContext<'a> {
    ...
    pub fn new(template: &'a Template) -> Self {
        ...
        for track in &template.tracks {
            ...
            let base_name = Self::normalize_base_name(&track.name);
            tracks_by_base_name
                .entry(base_name)
                .or_insert_with(Vec::new)
                .push(track);
    ...
    pub fn get_track(&self, name: &str) -> Option<&'a Track> {
    ...
    pub fn get_children(&self, parent_name: &str) -> Vec<&'a Track> {
...
impl TemplateFolder for AutoIncrementGroupFolder {
    fn transform_track(&mut self, mut track: Track, context: &FoldContext) -> Track {
        ...
        let parts: Vec<&str> = track.name.split_whitespace().collect();
        ...
        if is_incremented && existing_tracks.len() > 1 {
            ...
            let base_track_exists = existing_tracks.iter()
                .any(|t| FoldContext::normalize_base_name(&t.name) == base_name 
                     && t.name == base_name);
            ...
            if base_track_exists {
                ...
                track.remove_metadata("fts.parent");
    ...
    fn transform_track(&mut self, mut track: Track, context: &FoldContext) -> Track {
        ...
        if tracks_with_same_base.len() > 1 {
            ...
            if track.name.contains('.') {
                ...
                track.set_metadata("fts.use_playlist", "true");
            }
        }

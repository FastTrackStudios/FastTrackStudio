...
use crate::smart_template::visibility_manager::traits::{VisibilityManager, VisibilityState};
...
pub struct ReaperVisibilityManager {
    ...
    track_name_to_id: Mutex<HashMap<String, String>>,
}
...
impl ReaperVisibilityManager {
    ...
    pub fn register_track(&self, track_name: &str, track_id: String) {
        self.track_name_to_id.lock().unwrap().insert(track_name.to_string(), track_id);
    }
    
    /// Get track ID from track name
    fn get_track_id(&self, track_name: &str) -> Option<String> {
        self.track_name_to_id.lock().unwrap().get(track_name).cloned()
    }
    
    /// Get all track IDs for a template
    fn get_template_track_ids(&self, template: &Template) -> Vec<(String, String)> {
        template.tracks
            .iter()
            .filter_map(|track| {
...
impl VisibilityManager for ReaperVisibilityManager {
    ...
    fn apply_mode_visibility(
        ...
        let filtered_template = template.filter_by_mode_with_content(mode, |track| {
        ...
        let all_track_ids: Vec<String> = self.get_template_track_ids(template)
            .into_iter()
            .map(|(_, id)| id)
            .collect();
        
        // Get track IDs that should be visible in this mode
        let visible_track_ids: Vec<String> = self.get_template_track_ids(&filtered_template)
            .into_iter()
            .map(|(_, id)| id)
            .collect();

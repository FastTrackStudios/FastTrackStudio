    ...
    fn parse_item_properties(&self, name: &str) -> Result<Self::Output, Self::Error> {
        ...
        let is_guitar = props.group_prefix.as_deref() == Some("GTR")
            || props.sub_type.as_ref()
                .map(|st| st.iter().any(|s| s.eq_ignore_ascii_case("Guitar") || s.eq_ignore_ascii_case("AG")))
                .unwrap_or(false)
        ...
        if !is_guitar {
            return Err(TemplateParseError::NotMatch("Guitar Acoustic".to_string()));
        }
        ...
        if let Some(original) = &props.original_name {
            ...
            if matches_negative {
                return Err(TemplateParseError::NegativeMatch("Guitar Acoustic".to_string()));
            }
        }

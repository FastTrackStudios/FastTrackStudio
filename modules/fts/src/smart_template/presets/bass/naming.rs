...
/// Parse a track name into Bass properties
pub fn parse_bass(bass: &Bass, name: &str) -> Result<ItemProperties, BassParseError> {
    ...
    let is_bass = props.group_prefix.as_deref() == Some("Bass")
        ...
        || props.original_name.as_ref()
            .map(|n| {
                ...
                bass.config.patterns.iter()
                    .any(|p| name_lower.contains(&p.to_lowercase()))
                    || bass.config.pattern_categories.as_ref()
                        .map(|cats| {
                            cats.iter().any(|(_, cat)| {
                                cat.patterns.iter()
                                    .any(|p| name_lower.contains(&p.to_lowercase()))
    ...
    if let Some(original) = &props.original_name {
        ...
        let matches_negative = bass.config.negative_patterns.iter()
            .any(|p| {
...
mod tests {
    ...
    fn test_bass_parse() {
        let bass = Bass::new();
        assert!(bass.parse("Bass Guitar DI").is_ok());
        assert!(bass.parse("Kick").is_err());
    }
}

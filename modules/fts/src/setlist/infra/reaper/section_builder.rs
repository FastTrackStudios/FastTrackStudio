...
use super::parser::{parse_section_type_from_name, extract_number_from_name};
...
pub fn build_sections_from_regions(
    ...
    if let Some(ref count_in_marker) = song.count_in_marker {
        ...
        if count_in_start < song_start_pos {
            ...
            if let Some(count_in_section) = create_count_in_section(count_in_start_pos, song_start_pos_clone) {
...
pub fn build_sections_from_regions_simple(
    regions: &[Region],
    song: &Song,
) -> Vec<Section> {
    let song_start = song.start_position()

...
use daw::transport::{PlayState, RecordMode, Tempo, Transport, TransportActions, TransportError};
...
impl ReaperTransport {
    ...
    pub fn read_transport(&self) -> Result<Transport, TransportError> {
        ...
        let play_subdivision =
            ((play_beats_since_measure - play_beats_since_measure.floor()) * 1000.0).round() as i32;
        ...
        let edit_subdivision =
            ((edit_beats_since_measure - edit_beats_since_measure.floor()) * 1000.0).round() as i32;
...
impl TransportActions for ReaperTransport {
    fn play(&mut self) -> Result<String, TransportError> {
        ...
        reaper
            .medium_reaper()
            .main_on_command_ex(cmd_id, 0, ProjectContext::CurrentProject);
        Ok("Play".to_string())
    }

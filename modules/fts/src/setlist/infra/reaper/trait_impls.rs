...
use crate::lyrics::infra::traits::{
    LyricsData, LyricsReadError, LyricsWriteError, MidiEventData, MidiItemData, MidiTrackData,
    SlideData,
};
use crate::lyrics::infra::traits::{LyricsReader, LyricsWriter};
use crate::setlist::core::{Setlist, SetlistError, Song};
use crate::setlist::infra::traits::{CommandAdapter, SeekAdapter, SetlistBuilder};
use crate::setlist::{NavigationCommand, TransportCommand};
use daw::primitives::MusicalPosition;
use reaper_high::{Project as ReaperProject, Project as ReaperProjectHigh, Reaper};
use reaper_medium::{
    CommandId, MeasureMode, PositionInBeats, PositionInSeconds, ProjectContext, ProjectRef,
    SetEditCurPosOptions,
};
use tracing::{debug, error, info, warn};

use super::song_builder::{
    build_setlist_from_open_projects as build_setlist_impl, build_song_from_current_project,
};
...
pub fn switch_to_tab(tab_index: usize) -> Result<ReaperProjectHigh, String> {
    ...
    let current_idx = get_current_tab_index().ok_or_else(|| "No current tab".to_string())?;
    ...
    debug!(
        ...
        to_tab = tab_index,
        distance,
        direction = if forward_dist <= backward_dist {
            "forward"
        } else {
            "backward"
        },
    ...
    let final_tab =
        get_current_tab_index().ok_or_else(|| "Could not determine final tab index".to_string())?;
...
fn musical_pos_to_time(
    project: &ReaperProjectHigh,
    musical_pos: &MusicalPosition,
) -> Result<PositionInSeconds, String> {
    let reaper = Reaper::get();
    let medium_reaper = reaper.medium_reaper();
    ...
    let measure_offset =
        if let Some(offs_result) = medium_reaper.project_config_var_get_offs("projmeasoffs") {
            if let Some(addr) =
                medium_reaper.project_config_var_addr(project_context, offs_result.offset)
            {
...
impl SeekAdapter for ReaperProject {
    fn seek_to_section(&self, song_index: usize, section_index: usize) -> Result<(), String> {
        ...
        let setlist = build_setlist_impl(None).map_err(|e| {
        ...
        let target_pos = if let Some(start_pos) = &section.start_position {
            ...
            PositionInSeconds::new(absolute_time).map_err(|e| format!("Invalid position: {}", e))?
        };
    ...
    fn seek_to_time(&self, song_index: usize, time_seconds: f64) -> Result<(), String> {
        ...
        if !time_seconds.is_finite() {
            return Err(format!(
                "Invalid time value: {} (must be finite)",
                time_seconds
            ));
        }
        if time_seconds < 0.0 {
...
impl CommandAdapter for ReaperProject {
    fn execute_transport_command(&self, command: TransportCommand) -> Result<(), String> {
        ...
        let action_id = medium_reaper
            .named_command_lookup(action_name)
            .ok_or_else(|| {
    ...
    fn toggle_loop(&self) -> Result<(), String> {
        ...
        // Look up command ID by name
        let action_id = medium_reaper
            .named_command_lookup("Transport: Toggle repeat")
            .ok_or_else(|| {
...
impl LyricsReader for ReaperProject {
    fn read_lyrics_from_project(
        ...
        use crate::lyrics::infra::reaper::read::{
            LyricsData as ReaperLyricsData, LyricsReadError as ReaperLyricsReadError,
        };

        info!("Reading lyrics from project");

        let reaper_data: ReaperLyricsData =
            read_lyrics_from_project(*self).map_err(|e: ReaperLyricsReadError| {
                let error = match e {
                    ReaperLyricsReadError::LyricsFolderNotFound => {
                        LyricsReadError::LyricsFolderNotFound
                    }
                    ReaperLyricsReadError::SlidesTrackNotFound => {
                        LyricsReadError::SlidesTrackNotFound
                    }
...
impl LyricsWriter for ReaperProject {
    fn write_lyrics_to_project(
        ...
        if lyrics.sections.is_empty() {
            ...
            return Err(LyricsWriteError::WriteFailed(
                "Cannot write empty lyrics (no sections)".to_string(),
            ));
        }
        ...
        info!(
            song_index = song_idx,
            section_count, total_line_count, "Updating lyrics in project"
        );
        update_lyrics_in_reaper(song_idx, lyrics.clone()).map_err(|e| {
    ...
    fn update_lyrics_in_project(
        ...
        if song_index == usize::MAX {
            return Err(LyricsWriteError::WriteFailed(
                "Invalid song index".to_string(),
            ));
        }

        info!(song_index, "Updating lyrics in project for song");

        // Delegate to existing function
        update_lyrics_in_reaper(song_index, lyrics).map_err(|e| {

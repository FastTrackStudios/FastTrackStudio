...
#[cfg(feature = "reaper")]
use crate::lyrics::infra::reaper::read::{convert_lyrics_data_to_lyrics, read_lyrics_from_project};
...
pub fn build_song_from_current_project() -> Result<Song, SetlistError> {
    ...
    songs.into_iter().next().unwrap_or_else(|| {
        Err(SetlistError::validation_error(
            "No songs found in current project".to_string(),
        ))
    })
}
...
pub fn build_setlist_from_open_projects(
    ...
    for i in 0..128u32 {
        if let Some(result) = medium_reaper.enum_projects(ProjectRef::Tab(i), 512) {
            ...
            let markers = read_markers_from_project(&project).map_err(|e| {
                SetlistError::validation_error(format!("Failed to read markers: {}", e))
            })?;
            let regions = read_regions_from_project(&project).map_err(|e| {
                SetlistError::validation_error(format!("Failed to read regions: {}", e))
            })?;
    ...
    if !start_markers.is_empty() {
            ...
            if let Some(region) = song_region {
                ...
                if let Some(existing) = existing_setlist {
                    if let Some(existing_song) = existing.songs.iter().find(|s| {
                        s.name == song_name
                            && s.metadata.get("project_name").map(|s| s.as_str())
                                == Some(project_name)
                    }) {
                        debug!(
                            song_name = %song_name,
        ...
        if !songend_markers.is_empty() {
            ...
            for songend_marker in songend_markers {
                ...
                let renderstart = markers.iter().find(|m| {
                    m.name.trim() == "=START"
            ...
            if song_regions.is_empty() {
                ...
                if !regions.is_empty() {
                    ...
                    if regions.len() == 1 {
                        ...
                        let largest_region = regions.iter().max_by(|a, b| {
                        ...
                        if let Some(region) = largest_region {
                            ...
                            if contains_others {
                                ...
                                let synthetic_marker = Marker::from_seconds(
                ...
                for region in song_regions {
                    ...
                    let closest_start = markers.iter().find(|m| {
...
pub fn build_song_from_region(
    ...
    if let Some(proj_name) = context.project_name {
        song.metadata
            .insert("project_name".to_string(), proj_name.to_string());
    }
    ...
    let region_color = song_region.color.or_else(|| start_marker.color);
    ...
    let song_tempo_changes: Vec<TempoTimePoint> = tempo_changes_with_positions
        ...
        .map(|(song_relative_pos, tempo, time_sig)| {
            ...
            let position =
                Position::new(musical_pos, TimePosition::from_seconds(song_relative_pos));

            TempoTimePoint::new_full(
                position, tempo, None, // shape
                time_sig, None, // selected
                None, // bezier_tension
                None, // metronome_pattern
            )
        })
    ...
    match transport_adapter.read_transport() {
        Ok(transport) => {
            ...
            let mut project_with_transport =
                daw::project::Project::new(project_name_str, transport);
...
pub fn build_song_from_project_simple(
    ...
    if let Some(marker) = find_marker_by_name(markers, "=START") {
        ...
        debug!(
            position = marker.position_seconds(),
            "Found =START (RENDERSTART) marker"
        );
    }
    ...
    if let Some(region) = song_region {
        ...
        let region_color = region.color.or(start_marker_color);
...
fn calculate_measure_positions(project: &Project, song: &Song) -> Vec<Position> {
    ...
    for measure_num in song_start_musical..=song_end_musical {
        ...
        let beats_pos = PositionInBeats::new(0.0).unwrap_or_else(|_| PositionInBeats::ZERO);
        let measure_mode = MeasureMode::FromMeasureAtIndex(reaper_measure_index);
        let measure_time =
            medium_reaper.time_map_2_beats_to_time(project_context, measure_mode, beats_pos);

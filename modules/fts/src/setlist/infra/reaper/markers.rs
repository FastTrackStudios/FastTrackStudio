...
use daw::primitives::{MusicalPosition, Position, TimePosition, TimeRange};
        ...
        if let Some(bookmark) = project.find_bookmark_by_index(i) {
            ...
            if info.bookmark_type() == BookmarkType::Marker {
                ...
                let marker = Marker::new_full(
                    id, position, name, color, None, // flags - not available from basic_info
        ...
        if let Some(bookmark) = project.find_bookmark_by_index(i) {
            ...
            if info.bookmark_type() == BookmarkType::Region {
                if let Some(end_position) = info.region_end_position {
                    ...
                    let start_subdivision =
                        ((start_beats_since_measure - start_beats_since_measure.floor()) * 1000.0)
                            .round() as i32;
                    let start_subdivision = start_subdivision.clamp(0, 999);

                    let start_musical =
                        MusicalPosition::try_new(start_measure, start_beat, start_subdivision)
                            .map_err(|e| format!("Invalid musical position: {}", e))?;
                    ...
                    let end_subdivision =
                        ((end_beats_since_measure - end_beats_since_measure.floor()) * 1000.0)
                            .round() as i32;
                    let end_subdivision = end_subdivision.clamp(0, 999);

                    let end_musical =
                        MusicalPosition::try_new(end_measure, end_beat, end_subdivision)
                            .map_err(|e| format!("Invalid musical position: {}", e))?;

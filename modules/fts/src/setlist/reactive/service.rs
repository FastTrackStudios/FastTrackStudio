...
use super::{
    ActiveIndicesStreams, EventStreamSubject, LyricsStreams, SetlistReactiveStreams,
    SetlistStreams, SongStreams,
};
        ...
        let (changed, changes) = {
            ...
            let (changed, changes) = if let Some(old_setlist) = state.setlist.as_ref() {
                ...
                if old_setlist.songs.len() != setlist.songs.len() {
                    changes.push(format!(
                        "song_count: {} → {}",
                        old_setlist.songs.len(),
                        setlist.songs.len()
                    ));
                }

                // Check for song name changes
                for (i, (old_song, new_song)) in old_setlist
                    .songs
                    .iter()
                    .zip(setlist.songs.iter())
                    .enumerate()
                {
                    if old_song.name != new_song.name {
                        changes.push(format!(
                            "song[{}] name: {} → {}",
                            i, old_song.name, new_song.name
                        ));
                    }
                }

                // Check for new songs
                for (i, new_song) in setlist
                    .songs
                    .iter()
                    .enumerate()
                    .skip(old_setlist.songs.len())
                {
                ...
                (
                    true,
                    vec![format!("initial setlist: {} songs", setlist.songs.len())],
                )
            };
    ...
    fn emit_section_added(&self, song_index: usize, section_index: usize, section: Section) {
        ...
        *self.emitting.borrow_mut() = true;
        self.streams
            ...
            .section_added
            .borrow_mut()
            .next((song_index, section_index, section));
        *self.emitting.borrow_mut() = false;
    }
    ...
    fn emit_lyrics_annotations_changed(&self, song_index: usize, annotations: LyricsAnnotations) {
        ...
        *self.emitting.borrow_mut() = true;
        self.streams
            ...
            .borrow_mut()
            .next((song_index, annotations));
        *self.emitting.borrow_mut() = false;
    }

    fn emit_active_indices(
        &self,
        song: Option<usize>,

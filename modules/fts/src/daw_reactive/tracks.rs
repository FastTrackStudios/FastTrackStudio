...
/// REAPER implementation of TrackReactiveService
pub struct ReaperTrackReactiveService<S: SetlistProvider> {
...
impl<S: SetlistProvider> ReaperTrackReactiveService<S> {
    pub fn new(streams: TrackStreams, setlist_provider: Arc<S>) -> Self {
        Self {
    ...
    pub fn update_tracks_from_reaper(&self, reaper_project: ReaperProject) {
            ...
            .get_project_from_reaper(reaper_project)
        {
            ...
            let tracks = get_all_tracks(&reaper_project);
...
impl<S: SetlistProvider> TrackReactiveService for ReaperTrackReactiveService<S> {
                ...
                for (i, (old, new)) in old_tracks.iter().zip(tracks.iter()).enumerate() {
                    if old != new {
                        ...
                        if (old.volume - new.volume).abs() > 0.0001 {
                            track_change_parts
                                .push(format!("volume: {:.2} -> {:.2}", old.volume, new.volume));
                        }
                        if (old.pan - new.pan).abs() > 0.0001 {
                            track_change_parts
                                .push(format!("pan: {:.2} -> {:.2}", old.pan, new.pan));
                        }
                        if old.muted != new.muted {
                            track_change_parts
                                .push(format!("muted: {} -> {}", old.muted, new.muted));
                        }
                        if old.solo_state != new.solo_state {
                            track_change_parts.push(format!(
                                "solo: {:?} -> {:?}",
                                old.solo_state, new.solo_state
                            ));
                        }
                        if old.record_armed != new.record_armed {
                            track_change_parts.push(format!(
                                "armed: {} -> {}",
                                old.record_armed, new.record_armed
                            ));
                        }

                        if !track_change_parts.is_empty() {
                            track_changes.push(format!(
                                "track[{}] {} ({})",
                                i,
                                new.name,
                                track_change_parts.join(", ")
                            ));
                        }
                    }
                }
        ...
        let (changed, changes) = {
            ...
            let (changed, changes) = if track_index < tracks.len() {
                ...
                if old_track.solo_state != track.solo_state {
                    changes.push(format!(
                        "solo: {:?} -> {:?}",
                        old_track.solo_state, track.solo_state
                    ));
                }
                if old_track.record_armed != track.record_armed {
                    changes.push(format!(
                        "armed: {} -> {}",
                        old_track.record_armed, track.record_armed
                    ));
                }
                if old_track.automation_mode != track.automation_mode {
                    changes.push(format!(
                        "automation: {:?} -> {:?}",
                        old_track.automation_mode, track.automation_mode
                    ));
                }
                ...
                if old_track.is_folder != track.is_folder {
                    changes.push(format!(
                        "is_folder: {} -> {}",
                        old_track.is_folder, track.is_folder
                    ));
                }
                if old_track.folder_depth_change != track.folder_depth_change {
                    changes.push(format!(
                        "folder_depth_change: {:?} -> {:?}",
                        old_track.folder_depth_change, track.folder_depth_change
                    ));
                }
    ...
    fn emit_track(&self, project_id: String, track_index: usize, track: Track) {
        ...
        *emitting = true;
        ...
        self.streams
            ...
            .borrow()
            .clone()
            .next((project_id, track_index, track));
        *self.emitting.lock().unwrap() = false;
    }

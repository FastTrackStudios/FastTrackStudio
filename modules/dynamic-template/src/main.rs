...
fn print_tracks(tracks: &[daw::tracks::Track]) {
    ...
    for track in tracks {
        ...
        println!(
            "{}{}{}{}",
            indent_str, folder_marker, track.name.0, items_info
        );

        // Show items if any
        if !track.items.is_empty() {
            for item in &track.items {
                println!("{}      └─ \"{}\"", indent_str, item.name);
            }
        }
        ...
        match track.folder_depth_change {
            ...
            FolderDepthChange::ClosesLevels(n) => {
                ...
                depth = (depth as i32 + n).max(0) as usize;
            }

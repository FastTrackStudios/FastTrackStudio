...
use super::{Chord, ChordQuality, ChordFamily, Alteration, ChordDegree, Extensions, ExtensionQuality, SuspendedType};
use crate::primitives::{RootNotation, Interval};
...
impl std::fmt::Display for SemitoneSequenceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::AmbiguousChord(options) => write!(f, "Ambiguous chord, possibilities: {:?}", options),
        }
    }
}
...
fn analyze_chord_structure(semitones: &[u8]) -> Result<ChordInfo> {
    ...
    let mut intervals: Vec<u8> = pitch_classes.iter()
        .filter(|&&s| s != 0)
        .copied()
        .collect();
    ...
    let has_sus4 = pitch_classes.contains(&5) && !pitch_classes.contains(&4) && !pitch_classes.contains(&3);
    ...
    } else if has_aug_fifth && has_major_third && !has_perfect_fifth && !has_minor_seventh && !has_major_seventh {

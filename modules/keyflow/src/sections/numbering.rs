...
impl SectionNumberer {
    ...
    pub fn process_section(&mut self, section_type: SectionType, _measure_count: Option<usize>) -> Section {
        ...
        let (is_consecutive, last_split) = if let Some((last_key, last_num, last_split)) = &self.last_section {
            if last_key == &key {
    ...
    pub fn number_sections(&mut self, sections: &mut [Section]) {
        for i in 0..sections.len() {
            let numbered = self.process_section(sections[i].section_type.clone(), sections[i].measure_count);

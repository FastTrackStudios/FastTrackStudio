...
impl ChordMemory {
    ...
    pub fn remember_global(&mut self, root: &str, full_symbol: &str) {
        self.global.insert(root.to_lowercase(), full_symbol.to_string());
    }
...
mod tests {
    ...
    fn test_section_memory() {
        ...
        assert_eq!(memory.recall_section(&verse, "G"), Some("Gmaj13".to_string()));
        assert_eq!(memory.recall_section(&chorus, "G"), Some("Gmaj6".to_string()));
    }
    ...
    fn test_process_chord_with_quality() {
        ...
        assert_eq!(memory.recall_global("C"), Some("Cmaj7".to_string()));
        assert_eq!(memory.recall_section(&verse, "C"), Some("Cmaj7".to_string()));
    }
    ...
    fn test_process_chord_global_to_section_copy() {
        ...
        assert_eq!(result, "Cmaj7");
        assert_eq!(memory.recall_section(&chorus, "C"), Some("Cmaj7".to_string()));
    }
    ...
    fn test_section_redefinition_workflow() {
        ...
        assert_eq!(memory.recall_section(&verse, "C"), Some("Cmaj7".to_string())); // Copied to section
    }
    ...
    fn test_debug_csharp_major_harmonization() {
        ...
        for (i, chord) in chords.iter().enumerate() {
            println!("Degree {}: root={}, normalized={}", 
                i + 1, chord.root, chord.normalized);
        }

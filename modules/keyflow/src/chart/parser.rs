...
use super::types::{ChordInstance, Measure, ChartSection, KeyChange};
use super::cues::TextCue;
use crate::metadata::SongMetadata;
use crate::sections::{Section, SectionType, SectionNumberer};
use crate::key::Key;
use crate::time::{Tempo, TimeSignature, AbsolutePosition, MusicalDuration};
...
impl Chart {
    ...
    fn normalize_chord_case(token: &str) -> String {
        ...
        if first_char == 'I' || first_char == 'V' || first_char == 'i' || first_char == 'v' {
            ...
            if let Some(second_char) = token.chars().nth(1) {
                if second_char == 'I' || second_char == 'V' || second_char == 'i' || second_char == 'v' 
                   || second_char == '/' || second_char == '_' || second_char == '\'' {
    ...
    fn extract_trailing_apostrophes(token: &str) -> (String, usize) {
        ...
        let rhythm_start = token.find(|c: char| c == '/' || c == '_').unwrap_or(token.len());
    ...
    fn split_slash_chord(token: &str) -> (&str, Option<&str>) {
        ...
        if let Some(slash_pos) = token.find('/') {
            ...
            if slash_pos + 1 < token.len() {
                ...
                if after_slash.starts_with('/') || after_slash.starts_with('_') || after_slash.starts_with('\'') || after_slash.is_empty() {
                ...
                if bass_candidate.is_alphabetic() || bass_candidate.is_ascii_digit() {
                    ...
                    let bass_end = after_slash.find(|c: char| c == '/' || c == '_' || c == '\'')
    ...
    fn extract_root_from_token(token: &str) -> String {
        ...
        if first == 'I' || first == 'V' || first == 'i' || first == 'v' {
            let roman_part: String = chord_part.chars()
                ...
                if let Some(content) = inline_content {
                    ...
                    let measures = if content.is_empty() {
                        ...
                        if let Some(count) = measure_count {
                            ...
                            self.templates.recall_transposed(&section_type, self.current_key.as_ref()).unwrap_or_default()
                        }
                    } else {
                        // Has inline content - clear section memory and parse
                        self.chord_memory.clear_section(&section_type);
                        let parsed_measures = self.parse_section_measures(&[content], &section_type)?;
                        ...
                        if !matches!(section_type, 
                            ...
                            self.templates.store(&section_type, &parsed_measures, self.current_key.as_ref());
                        }
                    ...
                    idx = self.parse_section_content(lines, idx, section_type, measure_count, is_subsection)?;
                }
            } else {
    ...
    fn parse_section_content(
        ...
        while idx < lines.len() {
            ...
            if line_lower == "pre" || line_lower.starts_with("pre ") ||
               line_lower == "post" || line_lower.starts_with("post ") {
                break;
            }
        ...
        let mut measures = if content_lines.is_empty() {
            ...
            let parsed_measures = self.parse_section_measures(&content_lines, &section_type)?;
        ...
        if let Some(count) = measure_count {
            ...
            if actual_measures < count {
                ...
                eprintln!("Warning: Section {:?} has {} measures but specified {} - using actual content", 
    ...
    fn parse_section_measures(
        ...
        for line in lines {
            ...
            if line.trim().starts_with('@') {
                ...
                let mut line_measures = self.parse_chord_line(line, section_type)?;
    ...
    fn parse_chord_line(
        ...
        let tokens_str: Vec<&str> = line_to_parse.split_whitespace().collect();
        let mut measures: Vec<Measure> = Vec::new();
        ...
        for token_str in &tokens_str {
            ...
            let looks_like_key_sig = token_str.len() <= 3 && (token_str.starts_with('#') || token_str.starts_with('b'));
            ...
            if looks_like_key_sig {
                if let Ok(new_key) = Key::parse(token_str) {
                    ...
                    let key_change = KeyChange::new(position, self.current_key.clone(), new_key.clone(), section_index);
            ...
            match self.parse_chord_token(token_str, section_type, time_sig) {
                Ok(chord) => {
                    ...
                    if current_measure_beats + chord_beats > beats_per_measure + 0.001 { // small epsilon for float comparison
                    ...
                    if (current_measure_beats - beats_per_measure).abs() < 0.001 { // small epsilon for float comparison
                        measures.push(current_measure.clone());
                        current_measure = Measure::new();
                        current_measure_beats = 0.0;
                    }
        ...
        let final_repeat_count = match repeat_count {
            ...
            RepeatCount::Auto => {
                // Calculate auto-repeat based on section length
                // For now, just use 1 - we'll implement proper calculation later
                // TODO: Calculate based on section length vs phrase length
                1
            }
        };
    ...
    fn parse_chord_token(
        ...
        let full_symbol = self.chord_memory.process_chord(
            &root_from_token,
            chord_part,  // Use chord_part (after apostrophes stripped)
            &chord.normalized,
            section_type,
            is_override,
        ...
        let root_notation = RootNotation::from_string(&root_from_token)
    ...
    fn post_process(&mut self) {
        ...
        let mut sections_to_number: Vec<Section> = self.sections.iter()
            .map(|cs| cs.section.clone())
            .collect();
    ...
    fn apply_push_pull_adjustments(&mut self) {
        ...
        for section in &mut self.sections {
            ...
            let time_sig = if !section.measures.is_empty() {
                TimeSignature::new(section.measures[0].time_signature.0, section.measures[0].time_signature.1)
            } else {
            ...
            for (measure_idx, chord_idx, space_chord) in insertions.into_iter().rev() {
                section.measures[measure_idx].chords.insert(chord_idx, space_chord);
            }
            ...
            while i < all_chords.len() {
                if let Some((is_push, amount)) = all_chords[i].push_pull {
                    ...
                    if is_push && i > 0 {
                        ...
                        all_chords[i - 1].duration = MusicalDuration::from_beats(new_prev, time_sig);
                        ...
                        all_chords[i].duration = MusicalDuration::from_beats(new_curr, time_sig);
                        all_chords[i + 1].duration = MusicalDuration::from_beats(new_next, time_sig);
                    }
                }
                i += 1;
            }
    ...
    fn calculate_absolute_positions(&mut self) {
        ...
        for (section_idx, section) in self.sections.iter_mut().enumerate() {
            for measure in &mut section.measures {
                ...
                if measure.time_signature != (current_time_sig.numerator, current_time_sig.denominator) {
                    current_time_sig = TimeSignature::new(measure.time_signature.0, measure.time_signature.1);
                }
                
                // Assign positions to all chords in this measure
                for chord in &mut measure.chords {
                    // Set the chord's position to the current accumulated position
                    chord.position = AbsolutePosition::new(
                        current_position.clone(),
                        section_idx,
                    );
                    
                    // Add this chord's duration to the current position
                    let chord_duration_beats = chord.duration.to_beats(current_time_sig);
                    let new_position_beats = current_position.to_beats(current_time_sig) + chord_duration_beats;
                    current_position = MusicalDuration::from_beats(new_position_beats, current_time_sig);
                }
            }
        }
...
"#;
        ...
        // Verify section types
        assert_eq!(chart.sections[0].section.section_type, SectionType::Verse);
        assert!(matches!(chart.sections[1].section.section_type, SectionType::Pre(_)));
        assert_eq!(chart.sections[2].section.section_type, SectionType::Chorus);
        assert!(matches!(chart.sections[3].section.section_type, SectionType::Post(_)));
    }

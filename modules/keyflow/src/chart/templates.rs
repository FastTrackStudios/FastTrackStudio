...
impl TemplateManager {
    ...
    pub fn store_template(&mut self, section_type: &SectionType, measures: Vec<Measure>, key: Option<Key>) {
    ...
    pub fn recall_transposed(&self, section_type: &SectionType, target_key: Option<&Key>) -> Option<Vec<Measure>> {
        ...
        Some(template.iter().map(|measure| {
            Self::transpose_measure(measure, interval)
        }).collect())
    }
    ...
    fn transpose_measure(measure: &Measure, interval: i8) -> Measure {
        ...
        for chord in &mut transposed.chords {
            ...
            if let Some(resolved_note) = chord.root.resolved_note() {
                ...
                chord.full_symbol = format!("{}{}", new_note.name(), 
                    chord.full_symbol.chars().skip_while(|c| c.is_alphabetic() || *c == '#' || *c == 'b').collect::<String>());
            }
        }
...
mod tests {
    ...
    fn test_transposition_interval_calculation() {
        ...
        assert_eq!(TemplateManager::calculate_transposition_interval(&c_major, &g_major), -5);
        
        // C to F: up a perfect 4th (5 semitones)
        assert_eq!(TemplateManager::calculate_transposition_interval(&c_major, &f_major), 5);
        
        // G to C: shortest path is down 5 semitones
        assert_eq!(TemplateManager::calculate_transposition_interval(&g_major, &c_major), 5);
    }

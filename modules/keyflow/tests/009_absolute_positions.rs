...
"#;
    ...
    assert_eq!(section.measures.len(), 8, "Section should have exactly 8 measures");
    ...
    for i in 2..8 {
        assert!(!section.measures[i].chords.is_empty(), 
            "Measure {} should have a space chord", i);
        assert_eq!(section.measures[i].chords[0].full_symbol, "s",
            "Measure {} should be a space chord", i);
    }
    
    // Check absolute positions
    let all_chords: Vec<_> = section.measures.iter()
        .flat_map(|m| &m.chords)
        .collect();
...
"#;
    ...
    let chorus_first_chord = chart.sections[1].measures.iter()
        .flat_map(|m| &m.chords)
        .next()
...
"#;
    ...
    let chords: Vec<_> = section.measures.iter()
        .flat_map(|m| &m.chords)
        .collect();
...
"#;
    ...
    // Verse chords
    let verse_chords: Vec<_> = chart.sections[0].measures.iter()
        .flat_map(|m| &m.chords)
        .collect();
    ...
    let chorus_chords: Vec<_> = chart.sections[1].measures.iter()
        .flat_map(|m| &m.chords)
        .collect();
...
"#;
    ...
    for chord in chords {
        assert!(chord.position.total_duration.to_beats(chart.time_signature.unwrap()) >= 0.0);
    }
}

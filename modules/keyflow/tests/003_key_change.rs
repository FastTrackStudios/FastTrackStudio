...
"#;
    ...
    assert_eq!(format!("{}", key_change.from_key.as_ref().unwrap().root), "G");
    ...
    assert_eq!(chart.sections[3].section.section_type, SectionType::Post(Box::new(SectionType::Chorus)));
...
"#;
    ...
    // Test metadata
    assert_eq!(chart.metadata.title, Some("Key Change with Inference".to_string()));
    assert_eq!(chart.metadata.artist, Some("Demo".to_string()));

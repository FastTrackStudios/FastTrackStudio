...
Prereq: Phase A landed (commit 09db5ac on feat/threads-phase-a). Branch off it via `git worktree add` then EnterWorktree by path. fts-ui primitives + theme tokens only; no hex colors.
...
- fts-ui primitives only (Textarea, Button, StatusBadge with Success/Warning/Danger/Neutral variants only, Popover, IconButton). Theme tokens for colors; never hex/Tailwind palette like bg-red-500.

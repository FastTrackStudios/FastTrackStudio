...
- View component library on `knowledge-ui`: `KindList`, `KindKanban`, `KindCalendar`, `KindGallery`. Each is generic over a page-set. Wire all four. fts-ui primitives only, theme tokens only — `cargo clippy -p knowledge-ui` clean.
...
- fts-ui primitives only in UI; theme tokens never hex; dark-mode default; dumb components. (Per `AGENTS.md`.)

...
- **Icon foundation now, mass-migration later** — adopt the icon libraries and
  build Tags on them; migrating existing `fts_ui::lucide_dioxus` call sites is a
  separate sweep.
...
- **`dioxus-icons` v0.1** (DioxusLabs) — Lucide 1700+ as components
  ...
  current `fts_ui::lucide_dioxus`, so the later migration is near-mechanical.
...
5. *(later)* mass-migrate existing `fts_ui::lucide_dioxus` → `dioxus-icons`.

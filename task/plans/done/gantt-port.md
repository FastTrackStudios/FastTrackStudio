...
- Bar tooltips (fts-ui `Tooltip`): name + dates + progress
...
- Side-sheet editor via fts-ui `Sheet`
...
8. **Bar tooltips** (fts-ui has `Tooltip`; just wire it onto each bar).
9. **Inline edit of the bar label** (double-click → contenteditable).
10. **Date entry in the editor**. Currently read-only; needs
    fts-ui `DatePicker` integration + an `UpdateDates` event from the
    form.

    ...
    rsx! {
        div { class: "mx-auto flex max-w-6xl flex-col gap-8 p-6 lg:p-10",
            section { class: "flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between",
                div { class: "space-y-2",
                    Text { variant: TextVariant::Small, class: "uppercase tracking-[0.3em] text-primary font-semibold".to_string(), "Inbox" }
                ...
                Card { class: "min-w-32".to_string(),
                    CardContent { class: "p-4 pt-4 text-center".to_string(),
                        div { class: "text-2xl font-extrabold text-foreground", "{unread}" }
                        Text { variant: TextVariant::Small, class: "uppercase tracking-[0.2em] text-muted-foreground font-semibold".to_string(), "to triage" }
                    }
                }
            }
            ...
            if items.is_empty() {
                EmptyState {
                    message: "Inbox zero. Nothing waiting on a decision right now.".to_string(),
...
fn InboxRow(item: InboxItem) -> Element {
    ...
    rsx! {
        Card {
            CardHeader { class: "flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between".to_string(),
                HStack { gap: "3".to_string(), class: "items-start".to_string(),
                    ...
                    div { class: "flex flex-col gap-1",
                        ...
                        Text { variant: TextVariant::Small, class: "uppercase tracking-[0.2em] text-muted-foreground".to_string(),

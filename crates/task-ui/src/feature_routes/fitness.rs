//! WorkoutSession feature route. Holds a local `CrdtDoc` + `WorkoutSessionRepoLoro`,
//! syncs over WebSocket, drives `fitness-ui` dumb components.
...
#[component]
pub fn WorkoutSessionView() -> Element {
    let repo: Rc<WorkoutSessionRepoLoro> = use_hook(|| {
        let doc = CrdtDoc::ephemeral();
        Rc::new(WorkoutSessionRepoLoro::new(&doc))
    });
    let doc: Rc<CrdtDoc> = use_hook(|| Rc::new(CrdtDoc::from_loro(repo.doc().clone())));

    let mut items = use_signal::<Vec<WorkoutSession>>(Vec::new);
    ...
    let on_submit = {
        ...
        move |payload: WorkoutSessionCreate| {
            let repo = repo.clone();
            let tx = tx.clone();
            spawn_local(async move {

//! Location feature route. Holds a local `CrdtDoc` + `LocationRepoLoro`,
//! syncs over WebSocket, drives `location-ui` dumb components.

use std::rc::Rc;

use architect::Page;
use location_crdt::{LocationRepoLoro, CrdtDoc};
...
#[component]
pub fn LocationView() -> Element {
    let repo: Rc<LocationRepoLoro> = use_hook(|| {
        let doc = CrdtDoc::ephemeral();
        Rc::new(LocationRepoLoro::new(&doc))
    });
    let doc: Rc<CrdtDoc> =
        use_hook(|| Rc::new(CrdtDoc::from_loro(repo.doc().clone())));

    let mut items = use_signal::<Vec<Location>>(Vec::new);

//! Email feature route. Holds a local `CrdtDoc` + `EmailRepoLoro`,
//! syncs over WebSocket, drives `email-ui` dumb components.

use std::rc::Rc;

use architect::Page;
use email_crdt::{EmailRepoLoro, CrdtDoc};
...
pub fn EmailView() -> Element {
    let repo: Rc<EmailRepoLoro> = use_hook(|| {
        let doc = CrdtDoc::ephemeral();
        Rc::new(EmailRepoLoro::new(&doc))
    });
    let doc: Rc<CrdtDoc> =
        use_hook(|| Rc::new(CrdtDoc::from_loro(repo.doc().clone())));

    let mut items = use_signal::<Vec<Email>>(Vec::new);

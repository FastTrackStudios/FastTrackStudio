//! PantryItem feature route. Holds a local `CrdtDoc` + `PantryItemRepoLoro`,
//! syncs over WebSocket, drives `inventory-ui` dumb components.
...
#[component]
pub fn PantryItemView() -> Element {
    let repo: Rc<PantryItemRepoLoro> = use_hook(|| {
    ...
    rsx! {
        div { class: "mx-auto flex max-w-3xl flex-col gap-4 p-6 lg:p-10",
            h1 { class: "text-3xl font-bold", "Pantry (multiplayer)" }
            p { class: "text-xs text-slate-500", "{status_msg}" }

            PantryItemCreateForm { on_submit }
            PantryItemList { items: items(), on_delete }
        }
    }
}

//! Invoice feature route. Holds a local `CrdtDoc` + `InvoiceRepoLoro`,
//! syncs over WebSocket, drives `invoice-ui` dumb components.

use std::rc::Rc;

use architect::Page;
use invoice_crdt::{InvoiceRepoLoro, CrdtDoc};
...
pub fn InvoiceView() -> Element {
    let repo: Rc<InvoiceRepoLoro> = use_hook(|| {
    ...
    let mut items = use_signal::<Vec<Invoice>>(Vec::new);

//! Message feature route. Holds a local `CrdtDoc` + `MessageRepoLoro`,
//! syncs over WebSocket, drives `chat-ui` dumb components.
...
pub fn MessageView() -> Element {
    let repo: Rc<MessageRepoLoro> = use_hook(|| {

...
fn read_wav(path: &str) -> (Vec<f32>, u32) {
    ...
    while p + 8 <= d.len() {
        ...
        if id == b"fmt " {
            ...
            fmt = (f, u16::from_le_bytes([body[2], body[3]]),
    ...
    let ch = ch as usize; let by = (bits/8) as usize;
    ...
    for f in 0..frames {
        ...
        for c in 0..ch {
            ...
            let s = match (format, bits) {
                ...
                (1,24) => { let v=(data[o] as i32)|((data[o+1] as i32)<<8)|((data[o+2] as i32)<<16); let v=if v&0x80_0000!=0 {v| !0xFF_FFFF} else {v}; v as f32/8_388_608.0 }
...
fn fft(re: &mut [f32], im: &mut [f32]) {
    let n = re.len(); let mut j = 0usize;
    for i in 1..n { let mut bit=n>>1; while j&bit!=0 {j^=bit;bit>>=1;} j|=bit; if i<j {re.swap(i,j);im.swap(i,j);} }
    let mut len=2;
    while len<=n {
        let ang=-2.0*PI/len as f32; let (wr,wi)=(ang.cos(),ang.sin());
        let mut i=0;
        while i<n { let (mut cr,mut ci)=(1.0f32,0.0f32);
            for k in 0..len/2 { let a=i+k; let b=i+k+len/2;
                let tr=cr*re[b]-ci*im[b]; let ti=cr*im[b]+ci*re[b];
...
fn pitch(sig: &[f32], sr: u32, start_s: f32) -> (f32, String) {
    ...
    if s + n >= sig.len() { return (0.0, "?".into()); }
    let w = &sig[s..s + n];
    ...
    for lag in lag_min..=lag_max {
        let mut r = 0.0f32;
        for i in 0..n - lag { r += w[i] * w[i + lag]; }
        let r = r / energy;
        if r > best_r { best_r = r; best_lag = lag; }
    }
    (sr as f32 / best_lag as f32, note_name(sr as f32 / best_lag as f32))
}

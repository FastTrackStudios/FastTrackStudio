...
fn parse_css_descriptive_styx_mix_default() {
    ...
    let path = signal_root.parent().unwrap().join("sample-collector/specs/cinematic-strings.styx");
    if !path.exists() { return; }
    ...
    assert_eq!(le.low_latency.as_ref().unwrap().delay_for_velocity(30),  Some(100));
    assert_eq!(le.low_latency.as_ref().unwrap().delay_for_velocity(80),  Some(150));
}

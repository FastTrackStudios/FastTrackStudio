...
#[test]
fn css_brass_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Cinematic Series/Cinematic Studio Brass/zones.styx") else { return; };
...
fn css_solo_strings_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Cinematic Series/Cinematic Studio Solo Strings/zones.styx") else { return; };
...
fn css_woodwinds_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Cinematic Series/Cinematic Studio Woodwinds/zones.styx") else { return; };
    assert!(s.zones.len() > 100_000);
    let mics: std::collections::HashSet<_> = s.zones.iter().map(|z| z.mic.clone()).collect();
...
#[test]
fn css_piano_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Cinematic Series/Cinematic Studio Piano/zones.styx") else { return; };
...
fn pacific_brass_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Ocean Series/Pacific - Brass/zones.styx") else { return; };
...
fn pacific_ensemble_strings_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Ocean Series/Pacific - Ensemble Strings/zones.styx") else { return; };
    assert!(s.zones.len() > 50_000);
...
fn pacific_solo_strings_zones() {
    let Some(s) = try_load("/run/media/AudioHaven/Sampled/Orchestral/Ocean Series/Pacific - Solo Strings/zones.styx") else { return; };
    assert!(s.zones.len() > 10_000);
    let mics: std::collections::HashSet<_> = s.zones.iter().map(|z| z.mic.clone()).collect();

...
fn parse_mm2_hi_hat_styx() {
    ...
    assert!(spec.zones.iter().any(|z| z.key_min == 42 && z.articulation.starts_with("Closed")));
    assert!(spec.zones.iter().any(|z| z.key_min == 46 && z.articulation.starts_with("Open")));
    assert!(spec.zones.iter().any(|z| z.key_min == 44 && z.articulation.starts_with("Pedal")));
}

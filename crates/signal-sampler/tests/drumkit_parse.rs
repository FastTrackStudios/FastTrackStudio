...
fn parse_ggd_drumkit_styx() {
    ...
    let path = std::path::Path::new("/run/media/AudioHaven/Sampled/Drum Kits/GGD Modern and Massive/Snare/Gretsch Bell Brass 14x6.5/library.styx");
    ...
    // GM snare = D1 = 38
    assert!(spec.zones.iter().any(|z| z.key_min == 38), "no D1 snare zone");
}

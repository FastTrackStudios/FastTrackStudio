...
fn empty_chart_roundtrips() {
    let c = Chart::new();
    let bytes = vox_postcard::to_vec(&c).expect("encode empty chart");
    eprintln!("empty chart encoded {} bytes", bytes.len());
    let back: Chart = vox_postcard::from_slice(&bytes).expect("decode empty chart");
...
fn chart_with_section_memory_roundtrips() {
    ...
    let bytes = vox_postcard::to_vec(&c).expect("encode chart w/ map");
    eprintln!("chart w/ map encoded {} bytes", bytes.len());
    let back: Chart = vox_postcard::from_slice(&bytes).expect("decode chart w/ map");
...
fn error_roundtrips() {
    ...
    let bytes = vox_postcard::to_vec(&e).expect("encode error");
    eprintln!("error encoded {} bytes", bytes.len());
    let back: SessionServiceError = vox_postcard::from_slice(&bytes).expect("decode error");
    assert_eq!(e, back);
}
...
fn empty_setlist_roundtrips() {
    ...
    let bytes = vox_postcard::to_vec(&s).expect("encode empty setlist");
    eprintln!("empty setlist encoded {} bytes", bytes.len());
    let back: Setlist = vox_postcard::from_slice(&bytes).expect("decode empty setlist");
    assert_eq!(s, back);
}
...
fn named_setlist_roundtrips() {
    ...
    let bytes = vox_postcard::to_vec(&s).expect("encode named setlist");
    eprintln!("named setlist encoded {} bytes", bytes.len());
    let back: Setlist = vox_postcard::from_slice(&bytes).expect("decode named setlist");
    assert_eq!(s, back);
}

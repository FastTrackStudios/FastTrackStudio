...
pub fn resolve_wheel(
    _context: KeybindContext,
    _modifiers: &str,
    _horizontal: bool,
    _delta: i16,
) -> Option<String> {
    // TODO: Route wheel events through the InputProcessor's scroll binding system.
    // For now, wheel resolution is not yet migrated to the new processor.
    // The wheel_hook.rs caller should be updated to send ScrollEvents directly.
    None
}

...
use crate::types::{MuteAutomationBreakpoint, Track};
...
pub fn apply_mute_automation(
    ...
    for (i, wrapper) in wrappers.iter().enumerate() {
        let Some(envelope_block) = nth_child_by_raw(wrapper, 0x260a, 1) else {
            continue;
        };
        let bps = decode_envelope(envelope_block, data);
        if bps.is_empty() {
            continue;
        }
        // Apply to the track whose name pairs with this wrapper.
...
fn decode_envelope(block: &Block, data: &[u8]) -> Vec<MuteAutomationBreakpoint> {

...
async fn full_session_sync(ctx: &reaper_test::MultiDawTestContext) -> Result<()> {
    ...
    println!("  [master] Added item at 5.0s, len 2.0s: {}", &m_item_guid[..8]);
    ...
    assert!(found, "Item should sync to follower (found {} items)", f_items.len());
    ...
    match fx_result {
        Ok(fx) => {
            ...
            println!("  [master] Added ReaEQ: {}", &fx_guid[..8.min(fx_guid.len())]);
    ...
    if let Some(f_t) = f_tracks.iter().find(|t| t.name == "RenamedTrack") {
        if let Ok(Some(f_track)) = f_proj.tracks().by_guid(&f_t.guid).await {
            ...
            assert!(!f_sends.is_empty(), "Follower should have at least one send");
    ...
    assert!(after < before, "Follower should have fewer tracks after removal");

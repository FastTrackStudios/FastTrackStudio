...
pub fn MilestonesView() -> Element {
    ...
    let milestones = use_resource(move || {
    ...
    // Create the drafted milestone, then clear the form + refetch.
    let mut create = move || {
        ...
        due.set(String::new());
        spawn(async move {
            let _ = crate::feeds::create_milestone(&s, &t, project_id, due_date).await;
            refresh += 1;
        });
    };

    let (rows, load_err): (Vec<Milestone>, Option<String>) = match &*milestones.read() {
...
/// date + status badge.
#[component]
fn MilestoneRow(ms: Milestone, project_label: Option<String>) -> Element {

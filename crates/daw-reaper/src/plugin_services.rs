...
use daw_proto::{
    ActionRegistrationDispatcher, AudioEngineDispatcher, DawFileOpsDispatcher, HealthDispatcher,
    InputDispatcher, LiveMidiDispatcher, MidiDispatcher, PluginLoadingDispatcher,
    ProjectsDispatcher, RoutingDispatcher, ScreensetsDispatcher, ToolbarDispatcher,
    WindowGeometryDispatcher,
};
use daw_proto::{ext_state, fx, item, marker, region, take, tempo_map, track, transport};

use daw_proto::{
    action_registry_service_service_descriptor, audio_engine_service_service_descriptor,
    daw_file_service_service_descriptor, health_service_service_descriptor,
...
pub fn create_daw_handler() -> RoutedHandler {
    ...
    // FX broadcaster retired alongside the architect::rpc port.
    ...
    // Effects ported to architect::rpc — mounted via fx::serve(Reaper)
    ...
    RoutedHandler::new()
        ...
        .with(fx::descriptor(), fx::serve(crate::Reaper))

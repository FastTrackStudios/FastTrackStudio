...
pub trait FitnessService {
    ...
    /// Per-exercise history: the top set across each session that
    /// included this exercise. "Top set" means the heaviest weight ×
    /// rep combo (for strength) or longest duration (for cardio).
    async fn exercise_progress(
        &self,
        request: ExerciseProgressRequest,
    ) -> Result<ExerciseProgressView, VaultError>;

    /// Daily caloric balance: kcal consumed (from FoodLog) minus
    /// estimated kcal burned (from WorkoutSessions completed that day).

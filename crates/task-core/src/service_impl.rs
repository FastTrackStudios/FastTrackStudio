...
impl VaultServiceImpl {
    ...
    /// Aggregate uninvoiced billable time entries for a given client into an
    /// Invoice Ninja-ready draft. One line item per task with the task
    /// title as the notes and total hours × resolved rate.
    ///
    /// Entries with `invoiced_at: Some(_)` are skipped. Entries with no
    /// `end_time` (running timers) are skipped.
    pub async fn generate_invoice_draft(
        &self,
        client_name: &str,
        from: Option<chrono::DateTime<Utc>>,
        to: Option<chrono::DateTime<Utc>>,
        fallback_rate: Option<u32>,
    ) -> Result<(crate::client::Client, crate::provider::InvoiceDraft, Vec<String>), VaultError>
    {
        let client = self
            .find_client(client_name)
            .await
            .ok_or_else(|| VaultError::NotFound(format!("client '{client_name}'")))?;
        let invoice_ninja_id = client.invoice_ninja_id.clone().ok_or_else(|| {
            VaultError::NotFound(format!(
                "client '{client_name}' has no invoice_ninja_id — push to IN first"
            ))
        })?;
        ...
        // Group by (task, resolved_rate) — entries on the same task at
        // different rates produce separate line items so per-line math is
        // unambiguous and matches what the user would expect on the PDF.
        use std::collections::BTreeMap;
        let mut by_task_rate: BTreeMap<(String, u32), f64> = BTreeMap::new();
        let mut invoiced_entry_ids = Vec::new();
        for ctx in ctxs {
            let e = &ctx.entry;
            ...
            *by_task_rate
            ...
            invoiced_entry_ids.push(e.id.clone());
        }

        let lines: Vec<crate::provider::InvoiceLine> = by_task_rate
            .into_iter()
            .map(|((task, rate), hours)| crate::provider::InvoiceLine {
                product_key: task.clone(),
                notes: task,
                hours,
                rate_cents: rate,
        ...
        let draft = crate::provider::InvoiceDraft {
            client_invoice_ninja_id: invoice_ninja_id,
            po_number: None,
            date: None,
            due_date: None,
            public_notes: None,
            private_notes: Some(format!(
                "Generated from {} time entries via task-cli",
                invoiced_entry_ids.len()
            )),
            lines,
        };

        Ok((client, draft, invoiced_entry_ids))
    }

    /// Mark a set of time entries as invoiced against a specific IN invoice

...
pub async fn start(daw: Daw) -> eyre::Result<()> {
    ...
    tokio::spawn(handle_incoming(Arc::clone(&engine), incoming_rx));
    tokio::spawn(handle_peer_events(Arc::clone(&engine), peer_events_rx));
    tokio::spawn(poll_connect_peers(
        daw.clone(),
        Arc::clone(&mesh),
        session.peer_id.clone(),
    ));
    tokio::spawn(report_peer_count(daw.clone(), Arc::clone(&mesh)));
